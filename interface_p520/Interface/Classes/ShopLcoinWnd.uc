...
function Initialize()
{
	...
	GetTextureHandle((m_Windowname $ ".Buy_Wnd.BuyItemInfo_wnd.BtnInfoIcon_Texture")).HideWindow();
	GetButtonHandle((m_Windowname $ ".Buy_Wnd.BuyItemInfo_wnd.buyItemInfo_btn")).HideWindow();
	return;
}
...
function SetHomeSubItem(int Index, int homeListIndex)
{
	...
	Class'Interface.L2Util'.static.GetEllipsisString(itemNameStr, 224);
...
function HandleS_EX_PURCHASE_LIMIT_SHOP_ITEM_LIST_NEW()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_PURCHASE_LIMIT_SHOP_ITEM_LIST_NEW(packet))
	{
		return;
	}
