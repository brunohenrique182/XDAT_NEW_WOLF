...
{
	_InsertRecord(makeRecord(Class'Interface.DialogBox'.static.Inst().GetEditMessage(), param));
	return;
}
...
function HandleInsertDialogOK()
{
	...
	Title = Class'Interface.DialogBox'.static.Inst().GetEditMessage();
	if((Title == ""))
	{
		return;
	}
...
function OnAddClicked()
{
	...
	Class'Interface.DialogBox'.static.Inst().DelegateOnOK = HandleInsertDialogOK;
	return;
}
...
event OnClickButton(string strID)
{
	switch(strID)
	{
		case "BtnEditAdd":
			...
			{
				Class'Interface.DialogBox'.static.Inst().SetEditMessage(targetEditBox.GetString());
			}
			break;
		case "Copy_Btn":
			Copy();
		...
		case "BtnEditdel":
			...
			Class'Interface.DialogBox'.static.Inst().DelegateOnOK = DelSelected;
			break;
		case "BtnEditAlldel":
			...
			Class'Interface.DialogBox'.static.Inst().SetDefaultAction(EDefaultCancel);
			Class'Interface.DialogBox'.static.Inst().DelegateOnOK = DelAll;
			break;
		case "BtnEditUp":
			HandleUP();
			break;
