...
function API_C_EX_PLEDGE_CONTRIBUTION_LIST()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_PLEDGE_CONTRIBUTION_LIST(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(710, stream);
	return;
}
...
function API_C_EX_PLEDGE_ENEMY_INFO_LIST()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_PLEDGE_ENEMY_INFO_LIST(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(666, stream);
	return;
}
...
function API_C_EX_PLEDGE_ENEMY_DELETE(int EnemyPledgeSID)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_PLEDGE_ENEMY_DELETE(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(668, stream);
	return;
}
...
function Handle_S_EX_PLEDGE_CONTRIBUTION_LIST()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_PLEDGE_CONTRIBUTION_LIST(packet))
	{
		return;
	}
...
function API_C_EX_PLEDGE_ENEMY_REGISTER()
{
	...
	if((withEnemyPledgeNameStr != ""))
	{
		...
		if(!Class'Interface.UIPacket'.static.Encode_C_EX_PLEDGE_ENEMY_REGISTER(stream, packet))
		{
			return;
		}
		Class'Interface.UIPacket'.static.RequestUIPacket(667, stream);
	}
...
function Handle_S_EX_PLEDGE_ENEMY_INFO_LIST()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_PLEDGE_ENEMY_INFO_LIST(packet))
	{
		return;
	}
...
function HandleClickRefreshWarBtn()
{
	DisableRefreshWar();
	API_C_EX_PLEDGE_ENEMY_INFO_LIST();
	Class'Interface.L2UITimer'.static.Inst()._AddTimerOnce(5000)._DelegateOnEnd = EnableRefreshWar;
	return;
}
...
function HandleClickCancelEnemy()
{
	...
	Clan8_DeclaredListCtrl.GetRec(Index, Record);
	htmlStr = (htmlAddText(Record.LVDataList[0].szData, "", Class'Interface.L2UIColor'.static.Inst()._RGB2ZeroX(255, 204, 0)) @ GetSystemString(14454));
	if(((Record.nReserved2 > INT64(Class'Interface.UIData'.static.Inst().GetCurrentRealLocalTimeSec())) && (uData.CostLCoin > 0)))
	{
		...
		htmlStr = (((htmlStr $ "<br>") $ htmlAddText(Class'Interface.UIData'.static.Inst().getMakeTimeString(UITimeStruct), "", Class'Interface.L2UIColor'.static.Inst()._RGB2ZeroX(255, 204, 0))) @ htmlAddText(GetSystemString(14455), "", Class'Interface.L2UIColor'.static.Inst()._RGB2ZeroX(170, 153, 119)));
...
function HandleClickClanWar_OKBtn()
{
	...
	dayStr = htmlAddText(remainTimeString, "", Class'Interface.L2UIColor'.static.Inst()._RGB2ZeroX(255, 204, 0));
	costString = MakeCostString(string(uData.CostLCoin));
	costStr = htmlAddText(costString, "", Class'Interface.L2UIColor'.static.Inst()._RGB2ZeroX(255, 204, 0));
	DialogShowHtml(DialogModalType_Modalless, DialogType_OK, MakeFullSystemMsg(GetSystemMessage(13868), dayStr, costStr));
	Class'Interface.DialogBox'.static.Inst().DelegateOnOK = API_RequestEnemyPledgeRegister;
	Class'Interface.DialogBox'.static.Inst().m_hOwnerWnd.SetFocus();
	return;
}
...
function HandleClanEnelyList(UIPacket._L2PledgeEnemyInfo pledgeEnemyInfo)
{
	...
	{
		Record.nReserved2 = INT64(Class'Interface.UIData'.static.Inst().GetCurrentRealLocalTimeSec());
	}
	else
	{
...
function RichListCtrlRowData MakeRecordSkill(ItemID cID, int SkillLevel)
{
	...
	wMax = 267;
	Class'Interface.L2Util'.static.GetEllipsisString(Desc, wMax);
