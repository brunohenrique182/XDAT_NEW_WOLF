...
function Handle_S_EX_DELETE_HOMUNCLUS_DATA_RESULT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_DELETE_HOMUNCLUS_DATA_RESULT(packet))
	{
		return;
	}
	if((packet.Type == 1))
...
function Handle_S_EX_HOMUNCULUS_ACTIVATE_SLOT_RESULT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_HOMUNCULUS_ACTIVATE_SLOT_RESULT(packet))
	{
		return;
	}
	if((packet.Type == 1))
