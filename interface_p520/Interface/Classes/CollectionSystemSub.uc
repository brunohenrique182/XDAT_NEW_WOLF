...
{
	tObject = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject((1000 * 60), -1);
	tObject._DelegateOnTime = HandleDelegateOnTime;
	tObject._Pause();
	tObjectHandleList = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(1, -1);
	...
	tObjectHandleListModify = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(1, -1);
...
function bool makeRecord(int CollectionID, out RichListCtrlRowData outRecord)
{
	...
	Class'Interface.L2Util'.static.GetEllipsisString(ellipsedOptionName, 232);
