...
event OnClickButton(string strBtn)
{
	switch(strBtn)
	{
		...
		case "SaveBtn":
			...
			Class'Interface.UIListNoteWnd'.static.Inst()._SetString(NCEditBox0.GetString());
...
event OnShow()
{
	if(!_isInit)
	{
		timeObject = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(int(GetEditorString("Timer")), -1);
...
{
	NCEditBox0.SetString(Class'Interface.UIControlContextMenu'.static.GetInstance().menuObjects[Index].Name);
	Class'Interface.UIListNoteWnd'.static.Inst()._SetString(Class'Interface.UIControlContextMenu'.static.GetInstance().menuObjects[Index].Name);
	return;
}
