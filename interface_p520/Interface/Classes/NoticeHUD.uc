...
function HandleDialogOK()
{
	...
	switch(DialogGetID())
	{
		case 0:
			...
			break;
		case 1:
			Class'Interface.WorldSiegeWnd'.static.Inst().API_C_EX_WORLDCASTLEWAR_RETURN_TO_ORIGIN_PEER();
			break;
		case 2:
...
function SetWorldCastleSiegeHUDInfo(int castleID, int siegeState, int nowTime, int RemainTime)
{
	...
	Class'Interface.WorldSiegeRankingWnd'.static.Inst().SiegeInfo_Text.SetText(worldSiegeText.GetText());
	InputWorldCastleIDs(castleID);
	setWindowShow(worldsiegeNoticeWnd);
	worldsiegeRemainTime = Max(0, ((nowTime + RemainTime) - Class'Interface.UIData'.static.Inst().GetCurrentRealLocalTimeSec()));
	HandleTimeWorldSiegeRemain();
	worldsiegeNoticeWnd.KillTimer(5);
...
function HandleTimeWorldSiegeRemain()
{
	...
	Class'Interface.WorldSiegeBoardWnd'.static.Inst().Time_Txt.SetText(worldSiegeNoticeText.GetText());
	return;
}
...
function HandleEvent_S_EX_WORLDCASTLEWAR_CASTLE_SIEGE_HUD_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_WORLDCASTLEWAR_CASTLE_SIEGE_HUD_INFO(packet))
	{
		return;
	}
	switch(packet.nSiegeState)
	{
		case 1:
			if(IsInWorldSiege)
			{
				Class'Interface.WorldSiegeBoardWnd'.static.Inst().API_C_EX_WORLDCASTLEWAR_SIEGE_MAINBATTLE_HUD_INFO();
			}
			Class'Interface.WorldSiegeLauncherWnd'.static.Inst().m_hOwnerWnd.ShowWindow();
			break;
		default:
			Class'Interface.WorldSiegeLauncherWnd'.static.Inst().m_hOwnerWnd.HideWindow();
			Class'Interface.WorldSiegeBoardWnd'.static.Inst().EndWorldSiege();
			break;
	}
	IsInWorldSiege = true;
...
function HandleEvent_S_EX_WORLDCASTLEWAR_HOST_CASTLE_SIEGE_HUD_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_WORLDCASTLEWAR_HOST_CASTLE_SIEGE_HUD_INFO(packet))
	{
		return;
	}
...
function Handle_S_EX_SERVERWAR_NOTIFY_HOST_HUD_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_SERVERWAR_NOTIFY_HOST_HUD_INFO(packet))
	{
		return;
	}
...
function Handle_S_EX_SERVERWAR_NOTIFY_HUD_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_SERVERWAR_NOTIFY_HUD_INFO(packet))
	{
		return;
	}
...
function SetSkyTowerHUDInfo(int nFieldID, int nState, int nowTime, int RemainTime, int nNextStateBeginTime)
{
	...
	switch(nState)
	{
		...
		case 3:
			...
			{
				Class'Interface.SkyTowerWnd'.static.Inst().API_C_EX_SERVERWAR_LEADER_LIST(skyTowerFieldID);
				Class'Interface.SkyTowerWnd'.static.Inst().API_C_EX_SERVERWAR_SELECT_LEADER_INFO(skyTowerFieldID);
			}
			break;
		case 4:
		...
		case 5:
			...
			if(GetWindowHandle("SkyTowerWnd").IsShowWindow())
			{
				Class'Interface.SkyTowerWnd'.static.Inst().API_C_EX_SERVERWAR_SELECT_LEADER_INFO(skyTowerFieldID);
				Class'Interface.SkyTowerWnd'.static.Inst().API_C_EX_SERVERWAR_LEADER_LIST(skyTowerFieldID);
				Class'Interface.SkyTowerWnd'.static.Inst().API_C_EX_SERVERWAR_REWARD_ITEM_INFO(skyTowerFieldID);
				Class'Interface.SkyTowerWnd'.static.Inst().API_C_EX_SERVERWAR_REWARD_INFO(skyTowerFieldID);
			}
			break;
		default:
			break;
	...
	{
		SkyTowerRemainTime = Max(0, ((nowTime + RemainTime) - Class'Interface.UIData'.static.Inst().GetCurrentRealLocalTimeSec()));
		SkyTowerNoticeWnd.KillTimer(8);
...
function HandleOnClickSkyTower_Btn()
{
	...
	if((IsPlayerOnWorldRaidServer() == false))
	{
		...
		{
			Class'Interface.SkyTowerEnterWnd'.static.Inst().API_C_EX_SERVERWAR_FIELD_ENTER_USER_INFO(skyTowerFieldID);
		}
		return;
	}
...
function HandleOnEventNotice_Btn(int eventIndex)
{
	...
	{
		Class'Interface.EventNoticeEnterWnd'.static.Inst().ToggleShowNoticeEnterWnd(_eventNoticeInfos[eventIndex].EventID);
	}
	return;
}
...
function Handle_S_EX_MATCHINGINZONE_NOTIFY_HUD_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_MATCHINGINZONE_NOTIFY_HUD_INFO(packet))
	{
		return;
	}
	...
	RemainTime = (matchingInzoneMaintainTime - Class'Interface.UIData'.static.Inst().GetCurrentRealLocalTimeSec());
	Debug(("remainTime" @ string(RemainTime)));
	Debug((((((("S_EX_MATCHINGINZONE_NOTIFY_HUD_INFO" @ string(packet.nFieldID)) @ fieldUIData.Type) @ string(packet.nNowTime)) @ string(matchingInzoneMaintainTime)) @ string(Class'Interface.UIData'.static.Inst().GetCurrentRealLocalTimeSec())) @ string(RemainTime)));
	if((RemainTime > 0))
	{
...
function Handle_S_EX_ADEN_FORTRESS_SIEGE_HUD_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_ADEN_FORTRESS_SIEGE_HUD_INFO(packet))
	{
		return;
	}
	switch(packet.nSiegeState)
...
function Nt_S_EX_POPUP_EVENT_HUD()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_POPUP_EVENT_HUD(packet))
	{
		return;
	}
...
function API_C_EX_WORLDCASTLEWAR_MOVE_TO_HOST()
{
	...
	packet.nCastleID = 5;
	if(Class'Interface.UIPacket'.static.Encode_C_EX_WORLDCASTLEWAR_MOVE_TO_HOST(stream, packet))
	{
		Class'Interface.UIPacket'.static.RequestUIPacket(730, stream);
	}
	return;
}
...
function ShowTimeZoneExitDialog()
{
	...
	Class'Interface.UICommonAPI'.static.DialogSetID(2);
