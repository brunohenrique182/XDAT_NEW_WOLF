...
function SetTweenTwinkle(int Step)
{
	...
	{
		case 0:
			twinkleObject = Class'Interface.L2UITween'.static.Inst()._AddTweenTwinlkle(effectTexture00, 3.5000000, 0.5000000, 800.0000000, 0, 255, 0.0000000);
			break;
		case 1:
			twinkleObject = Class'Interface.L2UITween'.static.Inst()._AddTweenTwinlkle(effectTexture01, 3.5000000, 0.5000000, 800.0000000, 0, 255, 0.0000000);
			break;
		case 2:
			twinkleObject = Class'Interface.L2UITween'.static.Inst()._AddTweenTwinlkle(effectTexture02, 3.5000000, 0.5000000, 800.0000000, 0, 255, 0.0000000);
			break;
		default:
			break;
	}
...
{
	ReceiverID = Class'Interface.UIControlTextInput'.static.InitScript(GetWindowHandle("PostWriteWnd.ReceiverID"));
	...
	PostTitle = Class'Interface.UIControlTextInput'.static.InitScript(GetWindowHandle("PostWriteWnd.PostTitle"));
	...
	SafetyTradeAdenaTextBox = Class'Interface.UIControlTextInput'.static.InitScript(GetWindowHandle("PostWriteWnd.SafetyTradeAdena_win.SafetyTradeAdenaTextBox"));
...
function InitUIControlDialogAsset()
{
	...
	uicontrolDialogAssetScr = Class'Interface.UIControlDialogAssets'.static.InitScript(assetWnd);
	...
	else
	{
		if((int(currentPostType) == 0))
		{
			...
			else
			{
				...
				disableWnd.SetFocus();
				Class'Interface.DialogBox'.static.Inst().DelegateOnHide = HandleDelegateOnHide;
...
function HandleDialogOK()
{
	...
	if(DialogIsMine())
	{
		...
		else if((Id == 3333))
		{
			...
			if(((Adena > Class'Interface.DialogBox'.static.Inst().inputLimit) || (Adena < INT64(0))))
			{
...
function RQ_C_EX_POST_ITEM_FEE()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_POST_ITEM_FEE(stream, packet))
	{
		return;
	}
	Debug(("RQ_C_EX_POST_ITEM_FEE" @ string(packet.Items.Length)));
	Class'Interface.UIPacket'.static.RequestUIPacket(318, stream);
	return;
}
...
function RT_S_EX_POST_ITEM_FEE()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_POST_ITEM_FEE(packet))
	{
		return;
	}
