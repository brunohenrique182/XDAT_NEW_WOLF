...
function Initialize()
{
	...
	timeObjectEnchantDelay = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(ENCHANTING_TIME, 1);
	timeObjectEnchantDelay._DelegateOnTime = OnTimeEnchatDelay;
	UIControlNeedItemSelectMultiItem = Class'Interface.UIControlNeedItemSelectMultiItems'.static._InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".NeedItemWnd.UIControlNeedItemSelectMultiItem")));
...
function InitHennaGrouBtns()
{
	...
	while((i <= 4))
	{
		scr = Class'Interface.UIControlGroupButtonHighlighting'.static.InitScript(GetDyeSlot(i));
...
{
	needItemListNormalScr = Class'Interface.UIControlNeedItemList'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".NeedItemWnd")), 2);
...
event OnClickButton(string Name)
{
	switch(Name)
	{
		...
		case "EnchantBtn":
			...
			break;
		case "OpenBtn":
			Class'Interface.ItemJewelEnchantWnd'.static.Inst().ToggleShowWindow(dye);
			break;
		default:
			break;
	}
...
function SetSelectedProb()
{
	...
	{
		InstructionTxt.SetTextColor(Class'Interface.L2Util'.static.Inst().Red2);
...
function Handle_S_EX_NEW_HENNA_COMPOSE()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_NEW_HENNA_COMPOSE(composeResultPacket))
	{
		return;
	}
	bApiSending = false;
...
function Handle_S_EX_NEW_HENNA_COMPOSE_PROB_LIST()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_NEW_HENNA_COMPOSE_PROB_LIST(packet))
	{
		return;
	}
...
function API_C_EX_NEW_HENNA_COMPOSE()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_NEW_HENNA_COMPOSE(stream, packet))
	{
		return;
	}
	Debug(((("C_EX_NEW_HENNA_COMPOSE" @ string(packet.nSlotOneIndex)) @ string(packet.nSlotOneItemID)) @ string(packet.nSlotTwoItemID)));
	bApiSending = true;
	Class'Interface.UIPacket'.static.RequestUIPacket(751, stream);
	return;
}
...
function API_C_EX_NEW_HENNA_COMPOSE_PROB_LIST()
{
	...
	Class'Interface.UIPacket'.static.RequestUIPacket(897, stream);
	return;
}
...
auto state Begin
{
	function BeginState()
	{
		...
		Class'Interface.L2Util'.static.Inst().ItemRelationWindowHide(m_hOwnerWnd.m_WindowNameWithFullPath);
