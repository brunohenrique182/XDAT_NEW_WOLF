...
function SetPopupScript()
{
	...
	popupExpandScript = Class'Interface.UIControlDialogAssets'.static.InitScript(popupExpandWnd);
	return;
}
...
function API_C_EX_REPAIR_ALL_EQUIPMENT()
{
	...
	if(Class'Interface.UIPacket'.static.Encode_C_EX_REPAIR_ALL_EQUIPMENT(stream, packet))
	{
		Class'Interface.UIPacket'.static.RequestUIPacket(921, stream);
	}
...
function ParsePacket_S_EX_REPAIR_ALL_EQUIPMENT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_REPAIR_ALL_EQUIPMENT(packet))
	{
		return;
	}
