...
function Initialize()
{
	...
	enchantTimeObject = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(1, 1);
	enchantTimeObject._DelegateOnTime = delayAutoCall;
	timeObject = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(1000, 1);
...
function ParsePacket_S_EX_ENHANCED_ABILITY_OF_FIRE_INIT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_ENHANCED_ABILITY_OF_FIRE_INIT(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_ENHANCED_ABILITY_OF_FIRE_EXP_UP()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_ENHANCED_ABILITY_OF_FIRE_EXP_UP(packet))
	{
		return;
	}
	...
	if((packet.cResult > 0))
	{
		...
		Class'Interface.L2UITween'.static.Inst()._AddTweenTwinlkle(StatusBar_Text, 2.0000000, 0.5000000, 500.0000000);
...
function ParsePacket_S_EX_ENHANCED_ABILITY_OF_FIRE_LEVEL_UP()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_ENHANCED_ABILITY_OF_FIRE_LEVEL_UP(packet))
	{
		return;
	}
	...
	if((packet.cResult > 0))
	{
		...
		Class'Interface.L2UITween'.static.Inst()._AddTweenTwinlkle(SelectTypeWithLv_text, 2.0000000, 0.5000000, 1000.0000000);
	}
	else if((packet.cResult == 0))
	{
		GetWindowHandle("DethroneFireEnchantWnd").HideWindow();
