...
function InitControls()
{
	...
	rcTabGroupButton = Class'Interface.UIControlGroupButtonAssets'.static._InitScript(tabGroupButtonWindow);
...
function UpdateTeleportTagInfo(out array<TeleportTownInfo> townList)
{
	...
	localTimeSec = Class'Interface.UIData'.static.Inst().GetCurrentRealLocalTimeSec();
	i = 0;
...
function INT64 GetTeleportCost(INT64 cost, int UsableLevel, int UsableTransferDegree)
{
	...
	isFreeLevel = Class'Interface.UIData'.static.Inst().IsTeleportFreeLevel();
	if((_priceRacio > 0))
	{
...
{
	Class'Interface.TeleportWndMap'.static.Inst().UpdateTownZoneIcons(_teleportUIStateInfo.isRecommendType);
	return;
}

function UpdateMapDominionZoneIcons()
{
	Class'Interface.TeleportWndMap'.static.Inst().UpdateDominionZoneIcons();
	return;
}
...
function TeleportInfo GetTeleportInfo(int TeleportID, optional bool withTagInfo)
{
	...
	while((i < _teleportTotalList.Length))
	{
		...
		if((Info.Id == TeleportID))
		{
			if(withTagInfo)
			{
				Info.showTagType = GetValidTeleportTag(Info, Class'Interface.UIData'.static.Inst().GetCurrentRealLocalTimeSec());
			}
			return Info;
		}
		i++;
...
function Rq_EV_XML_TeleportFavoritesList()
{
	...
	Class'Interface.UIPacket'.static.RequestUIPacket(644, stream);
	return;
}
...
function Rq_C_EX_TELEPORT_FAVORITES_UI_TOGGLE(bool isOn)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_TELEPORT_FAVORITES_UI_TOGGLE(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(645, stream);
	return;
}
...
function Rq_C_EX_TELEPORT_FAVORITES_ADD_DEL(bool bAdd, int TeleportID)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_TELEPORT_FAVORITES_ADD_DEL(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(646, stream);
	return;
}
...
function RQ_C_EX_Teleport_UI()
{
	...
	Class'Interface.UIPacket'.static.RequestUIPacket(808, stream);
	return;
}
...
function Rs_S_EX_TELEPORT_UI()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_TELEPORT_UI(packet))
	{
		return;
	}
...
event OnTabGroupBtnClicked(string parentWndName, string strName, int Index)
{
	...
	ScrollToTopTownList();
	SetCurrentZoneListSelected();
	Class'Interface.TeleportWndMap'.static.Inst().HideMapIconTooltip();
	return;
}
...
event OnShow()
{
	...
	UpdateUIContols();
	Class'Interface.TeleportWndMap'.static.Inst().UpdateCurrentZoneInfo();
	Class'Interface.TeleportWndMap'.static.Inst().StartPlayerPositionTimer();
	Class'Interface.TeleportWndMap'.static.Inst().PlayMapIconAnimation();
	SetCurrentZoneListSelected();
...
event OnHide()
{
	Class'Interface.TeleportWndMap'.static.Inst().KillPlayerPositionTimer();
	Class'Interface.TeleportWndMap'.static.Inst().StopMapIconAnimation();
	teleportDialog.Hide();
	return;
}
	...
	switch(strID)
	{
		case "TownZone_ListCtrl":
			if((Index >= 0))
			{
				...
				Class'Interface.TeleportWndMap'.static.Inst().ShowMapIconTooltip(int(targetData.nReserved1));
			}
			else
			{
				Class'Interface.TeleportWndMap'.static.Inst().HideMapIconTooltip();
