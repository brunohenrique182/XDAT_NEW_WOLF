...
{
	scrollList = Class'Interface.UIControlTilelist'.static.InitScript(ScrollAreaWnd, 7, 1);
...
{
	needItemScript = Class'Interface.UIControlNeedItemList'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".NeedItemWnd")), 2, 2);
...
event OnClickButton(string strID)
{
	...
	{
		case "HelpWnd_Btn":
			Class'Interface.HelpWnd'.static.ShowHelp(109);
			break;
		case "close_Btn":
...
{
	if(Class'Interface.DialogBox'.static.Inst().m_hOwnerWnd.IsShowWindow())
	{
		if((("Transient." $ HomunculusWndScript.m_hOwnerWnd.m_WindowNameWithFullPath) == Class'Interface.DialogBox'.static.Inst().GetTarget()))
		{
...
function HandleDialogShow()
{
	...
	Class'Interface.DialogBox'.static.Inst().DelegateOnOK = HandleRevolutionProgress;
	Class'Interface.DialogBox'.static.Inst().DelegateOnHide = HandleOnNeedItemUpted;
	btn0.DisableWindow();
	ScrollAreaWnd.HideWindow();
	return;
}
...
function RQ_C_EX_HOMUNCULUS_EVOLVE()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_HOMUNCULUS_EVOLVE(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(892, stream);
	return;
}
...
function _RT_S_EX_HOMUNCULUS_EVOLVE()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_HOMUNCULUS_EVOLVE(packet))
	{
		return;
	}
	RequestedRelease();
...
function bool SetRequestLock()
{
	...
	Class'Interface.L2UITimer'.static.Inst()._AddTimerOnce(5000)._DelegateOnEnd = RequestedRelease;
	return true;
}
