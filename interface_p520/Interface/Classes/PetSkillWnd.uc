...
function InitControls()
{
	...
	tabGroupButton = Class'Interface.UIControlGroupButtonAssets'.static._InitScript(tabGroupWnd);
...
{
	if(Class'Interface.PetSkillInfoWnd'.static.Inst().Me.IsShowWindow())
	{
		Class'Interface.PetSkillInfoWnd'.static.Inst().UpdateSkillInfoControls();
	}
	return;
}
...
event OnPetSkillSlotInfoClicked(PetSkillSlotInfo__PetSkillWnd skillSlotInfo)
{
	...
	Class'Interface.PetSkillInfoWnd'.static.Inst().OpenWindow(skillSlotInfo, isShowSkillLearnPage);
	return;
}
...
event OnHide()
{
	Class'Interface.PetSkillInfoWnd'.static.Inst().Me.HideWindow();
	Class'Interface.PetSkillInfoWnd'.static.Inst().OnHide();
	_shortcutSKillList.Length = 0;
	_shortcutActionList.Length = 0;
	Me.KillTimer(1);
...
event OnReceivedCloseUI()
{
	if(Class'Interface.PetSkillInfoWnd'.static.Inst().Me.IsShowWindow())
	{
		Class'Interface.PetSkillInfoWnd'.static.Inst().OnReceivedCloseUI();
	}
	else
	{
