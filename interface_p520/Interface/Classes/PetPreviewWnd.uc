...
{
	if(Class'Interface.PetPreviewInfoWnd'.static.Inst().Me.IsShowWindow())
	{
		Class'Interface.PetPreviewInfoWnd'.static.Inst().UpdateSkillInfoControls();
	}
	return;
}
...
function Rq_C_EX_LOAD_PET_PREVIEW_BY_SID(int ServerID, int bWorldRaidServer)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_LOAD_PET_PREVIEW_BY_SID(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(912, stream);
	return;
}
...
function Rq_C_EX_LOAD_PET_PREVIEW_BY_DBID(int DBId)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_LOAD_PET_PREVIEW_BY_DBID(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(913, stream);
	return;
}
...
function Rs_EV_PetPreviewSkill(string param)
{
	...
	Class'Interface.L2Util'.static.GetSkill2ItemInfo(SkillInfo, skillSlotInfo.skillItemInfo);
...
event OnClickButton(string btnName)
{
	...
	{
		case "HelpBtn":
			Class'Interface.HelpWnd'.static.ShowHelp(15, 1);
			break;
		default:
			break;
	}
...
{
	Class'Interface.PetPreviewInfoWnd'.static.Inst().OpenWindow(skillSlotInfo);
	return;
}
...
event OnHide()
{
	Class'Interface.PetPreviewInfoWnd'.static.Inst().Me.HideWindow();
	Class'Interface.PetPreviewInfoWnd'.static.Inst().OnHide();
	ResetInfo();
	return;
}
...
event OnReceivedCloseUI()
{
	if(Class'Interface.PetPreviewInfoWnd'.static.Inst().Me.IsShowWindow())
	{
		Class'Interface.PetPreviewInfoWnd'.static.Inst().OnReceivedCloseUI();
	}
	else
	{
