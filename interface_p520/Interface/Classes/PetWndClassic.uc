...
function Initialize()
{
	...
	evolveDialogAsset = Class'Interface.UIControlDialogAssets'.static.InitScript(GetWindowHandle((dialogContainer.m_WindowNameWithFullPath $ ".UIControlDialogAsset")));
...
function HandlePetInfoUpdate()
{
	...
	Class'Interface.PetStatInfoWnd'.static.Inst().UpdateUIControls();
	return;
}
...
function API_C_EX_PET_EQUIP_ITEM(int nItemServerId)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_PET_EQUIP_ITEM(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(631, stream);
	return;
}
...
function API_C_EX_PET_UNEQUIP_ITEM(int nItemServerId)
{
	...
	packet.nItemServerId = nItemServerId;
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_PET_UNEQUIP_ITEM(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(632, stream);
	return;
}
...
function API_C_EX_EVOLVE_PET(optional int cDummy)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_EVOLVE_PET(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(641, stream);
