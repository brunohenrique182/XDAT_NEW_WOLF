...
{
	UIControlNeedItemListObj = Class'Interface.UIControlNeedItemList'.static.InitScript(GetWindowHandle("ItemJewelEnchantWnd.NeedItemWnd"));
...
{
	numberInputStepper = Class'Interface.UIControlNumberInputSteper'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".AutoEnchant_wnd.EnchantGoalSet_wndAsset")));
...
event OnLoad()
{
	...
	uiTimerObj = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject();
	uiTimerObj._DelegateOnEnd = NextProcess;
	iObject = Class'Interface.L2UIInventory'.static.Inst().NewObject();
...
event OnHide()
{
	...
	{
		Class'Interface.L2Util'.static.Inst().syncWindowLoc(m_hOwnerWnd.m_WindowNameWithFullPath, "HennaDyeEnchantWnd");
...
event OnClickButton(string Name)
{
	switch(Name)
	{
		...
		case "WindowHelp_BTN":
			...
			{
				Class'Interface.HelpWnd'.static.ShowHelp(72);
			}
			else
			{
				Class'Interface.HelpWnd'.static.ShowHelp(127);
			}
			break;
		case "cancelBtn":
...
function bool SetItemSlot2()
{
	...
	{
		if(Class'Interface.ItemJewelEnchantSubWnd'.static.Inst()._GetCanEnchantItemCurrent(nCommissionAdena, itemInfoArray))
		{
...
function ShowDialog(ItemInfo iInfo)
{
	...
	Class'Interface.DialogBox'.static.Inst().SetReservedItemInfo(iInfo);
	Class'Interface.DialogBox'.static.Inst().AnchorToOwner();
	Class'Interface.DialogBox'.static.Inst().DelegateOnOK = HandleDialogOK;
	Class'Interface.DialogBox'.static.Inst().DelegateOnHide = HandleDialogOnHide;
	return;
}
...
function ShowWarningDialog(string Msg)
{
	...
	Class'Interface.DialogBox'.static.Inst().AnchorToOwner();
	Class'Interface.DialogBox'.static.Inst().DelegateOnOK = HandleDialogWarningOK;
	Class'Interface.DialogBox'.static.Inst().DelegateOnHide = HandleDialogWarningCancel;
	return;
}
...
function HandleDialogOK()
{
	...
	Class'Interface.DialogBox'.static.Inst().GetReservedItemInfo(iInfo);
...
function bool MakeRowData(ItemInfo iInfo, out RichListCtrlRowData rowData)
{
	...
	Class'Interface.L2Util'.static.GetEllipsisString(itemNameStr, validTextWidth);
...
function RQ_C_EX_COMBINATION_PROB_LIST()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_COMBINATION_PROB_LIST(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(901, stream);
	return;
}
...
function RT_S_EX_COMBINATION_PROB_LIST()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_COMBINATION_PROB_LIST(packet))
	{
		return;
	}
	...
	ProbTxt.SetText(((GetSystemString(642) @ ":") @ Class'Interface.L2Util'.static.Inst().MakeDecimalPointString(Prob, 4, true, true)));
	return;
}
