...
event OnMouseOver(WindowHandle W)
{
	...
	{
		Class'Interface.L2UITween'.static.Inst().StopShake("BreakEquipmentNotice", 100);
		twinkleObject._Stop();
...
function Handle_S_EX_BREAK_EQUIPMENT_NOTI()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_BREAK_EQUIPMENT_NOTI(packet))
	{
		return;
	}
	...
	if((nBreakCount > 0))
	{
		twinkleObject = Class'Interface.L2UITween'.static.Inst()._AddTweenTwinlkle(AlarmTexture, 4.5000000, 0.5000000, 800.0000000, 0, 255, 0.0000000);
		Class'Interface.L2UITween'.static.Inst().StartShake("BreakEquipmentNotice", 4, 1000, small, 0, 100);
