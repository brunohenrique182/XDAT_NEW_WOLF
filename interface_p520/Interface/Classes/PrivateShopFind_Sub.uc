...
{
	SubGroupButtonAsset = Class'Interface.UIControlGroupButtonAssets'.static._InitScript(GetWindowHandle((m_Windowname $ ".SubUIControlGroupButtonAsset")));
...
function ParsePacket_S_EX_PRIVATE_STORE_SEARCH_ITEM()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_PRIVATE_STORE_SEARCH_ITEM(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_PRIVATE_STORE_SEARCH_HISTORY()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_PRIVATE_STORE_SEARCH_HISTORY(packet))
	{
		return;
	}
	i = 0;
...
function API_C_EX_PRIVATE_STORE_SEARCH_LIST(string sSearchWord, int cStoreType, int cItemType, int cItemSubtype, bool bCollectionUse)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_PRIVATE_STORE_SEARCH_LIST(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(741, stream);
...
function API_C_EX_PRIVATE_STORE_BUY_SELL(int nUserSID)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_PRIVATE_STORE_BUY_SELL(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(845, stream);
