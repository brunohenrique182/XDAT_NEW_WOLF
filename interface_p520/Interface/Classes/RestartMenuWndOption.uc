...
function RT_S_EX_USER_RESTART_LOCKER_LIST()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_USER_RESTART_LOCKER_LIST(packet))
	{
		return;
	}
...
function RT_S_EX_USER_RESTART_LOCKER_UPDATE()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_USER_RESTART_LOCKER_UPDATE(packet))
	{
		return;
	}
	tObject._Stop();
...
function RQ_C_EX_USER_RESTART_LOCKER_UPDATE(UIEventManager.RestartPoint pnt, int ClassID, bool bLock)
{
	...
	tObject._Reset();
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_USER_RESTART_LOCKER_UPDATE(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(783, stream);
	return;
}
...
function InitTimer()
{
	tObject = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(5000);
