...
function Initialize()
{
	rotateTObject = Class'Interface.L2UITween'.static.Inst()._AddTweenRotate(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".CircleEffect_Btn")), 9, 360.0000000, 80000.0000000, 0.0000000, true);
...
{
	tObject = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(1, -1);
	tObject._DelegateOnStart = HandleDelegateOnStart;
	tObject._DelegateOnTime = HandleDelegateOnTime;
	tObject._Stop();
	tObjectEnd = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(5000, 1);
...
{
	uicontrolTextInputScr = Class'Interface.UIControlTextInput'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".Input_TextBox")));
...
function RQ_C_EX_RAID_AUCTION_CANCEL_BID()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_RAID_AUCTION_CANCEL_BID(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(917, stream);
	return;
}
...
function RQ_C_EX_RAID_AUCTION_REGISTER(INT64 Amount)
{
	...
	packet.nID = CurrentID;
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_RAID_AUCTION_BID(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(916, stream);
	return;
}
...
function Rt_S_EX_RAID_AUCTION_BID_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_RAID_AUCTION_BID_INFO(packet))
	{
		return;
	}
...
function Rt_S_EX_RAID_AUCTION_RESULT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_RAID_AUCTION_RESULT(packet))
	{
		return;
	}
	Time_Txt.HideWindow();
...
{
	Class'Interface.L2UITween'.static.Inst()._AddTweenTwinlkle(DscText_Html, 2.0000000, 0.5000000, 500.0000000);
	return;
}
...
function ShakeHtmlText()
{
	...
	Class'Interface.L2UITween'.static.Inst().StartShake(DscText_Html.m_WindowNameWithFullPath, 4, 500, small, 0, 1010101);
	return;
}

function InitTweenObject()
{
	TweenObject = new Class'Interface.L2UITweenObject';
