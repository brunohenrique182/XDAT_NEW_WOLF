...
function initGroupButton()
{
	...
	TopGroupButtonAsset = Class'Interface.UIControlGroupButtonAssets'.static._InitScript(GetWindowHandle("HeroBookWnd.HeroBookList_Tab"));
...
{
	Class'Interface.HelpWnd'.static.ShowHelp(64);
	return;
}
...
function API_C_EX_HERO_BOOK_ENCHANT()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_HERO_BOOK_ENCHANT(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(806, stream);
	return;
}
...
function ParsePacket_S_EX_HERO_BOOK_ENCHANT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_HERO_BOOK_ENCHANT(packet))
	{
		return;
	}
