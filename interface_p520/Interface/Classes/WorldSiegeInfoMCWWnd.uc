...
event OnShow()
{
	...
	Class'Interface.WorldSiegeWnd'.static.Inst().m_hOwnerWnd.HideWindow();
...
{
	if((!m_hOwnerWnd.IsShowWindow() && !Class'Interface.WorldSiegeWnd'.static.Inst().m_hOwnerWnd.IsShowWindow()))
	{
		return;
	}
	switch(Event_ID)
...
function HandleSiegeInfoClanList(string param)
{
	...
	if((clanID == myClanID))
	{
		if((clanID == ClanWndClassicNew(GetScript("ClanWndClassicNew")).m_clanID))
		{
			...
			Class'Interface.WorldSiegeWnd'.static.Inst().SetRecruit(MercenaryRecruit);
...
function UpdateAttackButton()
{
	...
	if((Class'Interface.NoticeHUD'.static.Inst().worldsiegeState != 0))
	{
		return;
	}
...
{
	return Class'Interface.WorldSiegeWnd'.static.Inst().OwnerPledgeID;
}
...
function API_C_EX_WORLDCASTLEWAR_CASTLE_SIEGE_ATTACKER_LIST()
{
	...
	packet.nCastleID = GetCastleID();
	if(Class'Interface.UIPacket'.static.Encode_C_EX_WORLDCASTLEWAR_CASTLE_SIEGE_ATTACKER_LIST(stream, packet))
	{
		Class'Interface.UIPacket'.static.RequestUIPacket(735, stream);
	}
	return;
}
...
function API_C_EX_WORLDCASTLEWAR_CASTLE_SIEGE_JOIN()
{
	...
	if(Class'Interface.UIPacket'.static.Encode_C_EX_WORLDCASTLEWAR_CASTLE_SIEGE_JOIN(stream, packet))
	{
		Class'Interface.UIPacket'.static.RequestUIPacket(734, stream);
	}
	return;
}
