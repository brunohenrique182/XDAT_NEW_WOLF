...
function API_C_EX_SUBJUGATION_LIST()
{
	...
	Class'Interface.UIPacket'.static.RequestUIPacket(704, stream);
...
function ParsePacket_S_EX_SUBJUGATION_LIST()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_SUBJUGATION_LIST(packet))
	{
		return;
	}
...
function API_C_EX_SUBJUGATION_RANKING(int nID)
{
	...
	packet.nID = nID;
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_SUBJUGATION_RANKING(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(705, stream);
	return;
}
...
function ParsePacket_S_EX_SUBJUGATION_RANKING()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_SUBJUGATION_RANKING(packet))
	{
		return;
	}
	if((packet.nRank == 0))
