...
function InitDefaultSetting()
{
	...
	m_Windowname = Class'Interface.L2Util'.static.GetFullPath(Me);
...
function InitScrollPanel()
{
	...
	scrollTesterV = Class'Interface.UIControlTilelist'.static.InitScript(GetWindowHandle((m_WindowName_ClanEmblemManageWnd $ ".ScrollAreaWndV")), 5, 11, true);
...
function HandleDelegateOnRenderClickV(string BTNID, int rendererIndex, int itemIndex)
{
	...
	Class'Interface.DialogBox'.static.Inst().AnchorToOwner(-320, -4);
	Class'Interface.DialogBox'.static.Inst().DelegateOnOK = HandleDialogOK;
	Class'Interface.DialogBox'.static.Inst().DelegateOnHide = HandleDialogHide;
	Class'Interface.DialogBox'.static.Inst().SetReservedInt(GetSelectedPresetEmblemID());
	return;
}
...
{
	Debug(("OKOKOK" @ string(Class'Interface.DialogBox'.static.Inst().GetReservedInt())));
	API_RequestClanRegisterCrestPreset(Class'Interface.DialogBox'.static.Inst().GetReservedInt());
	return;
}
