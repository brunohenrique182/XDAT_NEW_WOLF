...
function InitControls()
{
	...
	tabButtons = new Class'Interface.UIControlGroupButtons';
	...
	tabButtons._setButtonValue(1, 5);
	tabButtons._setButtonValue(2, 4);
	tabButtons._setButtonValue(3, 3);
	tabButtons._setButtonValue(4, 2);
	tabButtons._setButtonValue(5, 1);
	tabButtons.DelegateOnClickButton = OnClickTabButton;
	scrollTileList = Class'Interface.UIControlTilelist'.static.InitScript(GetWindowHandle((ownerFullPath $ ".RelicSlotList_Wnd")), 4, 4, true);
	...
	while((i < (4 * 4)))
	{
		...
		slotObject = new Class'Interface.RelicWndSlot';
		slotObject.Init(itemRendererWnd);
		rendererObjectList[rendererObjectList.Length] = slotObject;
		i++;
	}
	Class'Interface.RelicWnd'.static.Inst().DelegateChangeRelicList = OnChangeRelicList;
	Class'Interface.RelicWnd'.static.Inst().DelegateChangeRelicActive = OnChangeRelicActive;
	Class'Interface.RelicWnd'.static.Inst().DelegateChangeCombineStuffList = OnChangeCombineStuffList;
	Class'Interface.RelicWnd'.static.Inst().DelegateChangeUpgradeStuffList = OnChangeUpgradeStuffList;
	hideDisableWnd();
	SetDefaultTab();
	return;
}
...
function UpdateList(optional bool needListReset)
{
	...
	_relicInfos = Class'Interface.RelicWnd'.static.Inst().GetRelicInfos();
	_uiInfo = Class'Interface.RelicWnd'.static.Inst().GetRelicUIInfo();
	UpdateRelicInfos();
	...
	{
		if(((int(_uiInfo.uiState) == 3) && (int(GetSelectedTabGrade()) == GetRelicsUncombinableGrade())))
		{
	...
	{
		if((Class'Interface.RelicWnd'.static.Inst().GetUpgradeInfo().targetRelicId != 0))
		{
...
function SetDefaultInfo()
{
	if((_relicInfos.Length > 0))
	{
		Class'Interface.RelicWnd'.static.Inst().OnSelectTileList(_relicInfos[0]);
	}
	return;
}
...
function OnClickTileList(string BTNID, int rendererIndex, int itemIndex)
{
	...
	{
		Class'Interface.RelicWnd'.static.Inst().OnClickTileList(_relicInfos[itemIndex]);
	}
	return;
}
...
function OnSelectTileList(string itemRendererID, int rendererIndex, int itemIndex)
{
	if((itemIndex < _relicInfos.Length))
	{
		Class'Interface.RelicWnd'.static.Inst().OnSelectTileList(_relicInfos[itemIndex]);
	}
	return;
}
...
{
	if(Class'Interface.RelicWnd'.static.Inst().IsOpenWindow())
	{
		UpdateList();
	}
	return;
...
function OnChangeRelicActive()
{
	if(Class'Interface.RelicWnd'.static.Inst().IsOpenWindow())
	{
		UpdateList();
		SetSelected(0, true);
		Class'Interface.RelicWnd'.static.Inst().relicInfoScript.PlayActiveAnimTex();
	}
	return;
}
...
function UpdateRelicInfos()
{
	...
	upgradeInfo = Class'Interface.RelicWnd'.static.Inst().GetUpgradeInfo();
	if(((int(_uiInfo.uiState) == 3) || ((int(_uiInfo.uiState) == 2) && (upgradeInfo.targetRelicId != 0))))
	{
		isStuffMode = true;
	}
	...
	while((i < _relicInfos.Length))
	{
		...
		if(((int(_uiInfo.uiState) == 3) && (tmpRelicInfo.Data.Grade == uncombineGrade)))
		{
			i++;
			continue;
		}
		if(((int(_uiInfo.uiState) == 2) && (tmpRelicInfo.isEnabled == false)))
		{
			i++;
			continue;
		}
		...
		if((isStuffMode == true))
		{
			...
			{
				tmpRelicInfo.Count = (tmpRelicInfo.Count - INT64(Class'Interface.RelicWnd'.static.Inst().GetRegisteredCombineStuffCount(tmpRelicInfo.relicId)));
			}
			else
			{
				tmpRelicInfo.Count = (tmpRelicInfo.Count - INT64(Class'Interface.RelicWnd'.static.Inst().GetRegisteredUpgradeStuffCount(tmpRelicInfo.relicId)));
			}
			if((tmpRelicInfo.Count <= INT64(0)))
			{
