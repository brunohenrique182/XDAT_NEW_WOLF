...
function ParsePacket_S_EX_ITEM_ANNOUNCE_SETTING()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_ITEM_ANNOUNCE_SETTING(packet))
	{
		return;
	}
...
function OnAskPartyLootingModify(string a_Param)
{
	...
	Class'Interface.UICommonAPI'.static.DialogSetID(9);
	Class'Interface.UICommonAPI'.static.DialogSetParamInt64(INT64((10 * 1000)));
	Class'Interface.UICommonAPI'.static.DialogSetDefaultCancle();
	ParseString(a_Param, "LeaderName", LeaderName);
	ParseInt(a_Param, "LootingScheme", LootingScheme);
	Schemestr = util.getLootingString(LootingScheme);
	Class'Interface.UICommonAPI'.static.DialogShowWithTarget(DialogModalType_Modalless, DialogType_Progress, MakeFullSystemMsg(GetSystemMessage(3134), Schemestr), m_Windowname);
	return;
}
...
function API_C_EX_SAVE_ITEM_ANNOUNCE_SETTING(bool bAnonymity)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_SAVE_ITEM_ANNOUNCE_SETTING(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(625, stream);
	return;
}
...
function HandleChatFontSizeSaved(string param)
{
	...
	Class'Interface.ChatWnd'.static.Inst()._SetChangeFont(int(param));
	return;
}
