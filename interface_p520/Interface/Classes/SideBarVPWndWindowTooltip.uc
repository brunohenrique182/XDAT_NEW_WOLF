...
{
	timeObject = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(1000, 60);
...
function RT_S_EX_VITALITY_KEEP_VITALPOINT_INFO()
{
	...
	return;
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_VITALITY_KEEP_VITALPOINT_INFO(packet))
	{
		return;
	}
...
function TestRemainTime()
{
	...
	{
		SetEndDateTime((Class'Interface.UIData'.static.Inst().GetCurrentRealLocalTimeSec() + Rand(70)));
	}
	return;
}
...
function SetEndDateTime(int endDateTime)
{
	...
	SetRemainTimer((endDateTime - Class'Interface.UIData'.static.Inst().GetCurrentRealLocalTimeSec()));
	return;
}
...
{
	RemainTime_Txt.SetText(Class'Interface.L2Util'.static.Inst().getTimeStringBySec3((timeObject._maxCount - curCount)));
	return;
}
