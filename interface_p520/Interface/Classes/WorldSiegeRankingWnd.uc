...
function Handle_S_EX_WORLDCASTLEWAR_HOST_CASTLE_SIEGE_HUD_INFO()
{
	...
	WorldSiegeDate02_Text.SetText((((((((((((GetSystemString(13888) $ "\\n") $ string(L2UITime.nYear)) $ ".") $ string(L2UITime.nMonth)) $ ".") $ string(L2UITime.nDay)) @ "[") $ Class'Interface.UIData'.static.Inst().Int2Str(L2UITime.nHour)) $ ":") $ Class'Interface.UIData'.static.Inst().Int2Str(L2UITime.nMin)) $ "]"));
	return;
}
...
function Handle_S_EX_WORLDCASTLEWAR_HOST_CASTLE_SIEGE_RANKING_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_WORLDCASTLEWAR_HOST_CASTLE_SIEGE_RANKING_INFO(packet))
	{
		return;
	}
	if(!ChkCastleID(packet.nCastleID))
...
function Handle_S_EX_WORLDCASTLEWAR_CASTLE_SIEGE_RANKING_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_WORLDCASTLEWAR_CASTLE_SIEGE_RANKING_INFO(packet))
	{
		return;
	}
	if(!ChkCastleID(packet.nCastleID))
	...
	lstPersonalRankingList = packet.lstPersonalRankingList;
	CastleName_Text.SetText(GetCastleName(packet.nCastleID));
	Debug((((("Handle_S_EX_WORLDCASTLEWAR_CASTLE_SIEGE_RANKING_INFO stp :" @ GetCastleName(packet.nCastleID)) @ string(Class'Interface.NoticeHUD'.static.Inst().worldsiegeState)) @ string(packet.lstPledgeRankingList.Length)) @ string(packet.lstPersonalRankingList.Length)));
	MakeList();
	if((Class'Interface.NoticeHUD'.static.Inst().worldsiegeState == 1))
	{
...
function API_C_EX_WORLDCASTLEWAR_CASTLE_INFO()
{
	...
	if(Class'Interface.UIPacket'.static.Encode_C_EX_WORLDCASTLEWAR_CASTLE_INFO(stream, packet))
	{
		Class'Interface.UIPacket'.static.RequestUIPacket(732, stream);
...
function API_C_EX_WORLDCASTLEWAR_HOST_CASTLE_SIEGE_RANKING_INFO()
{
	...
	packet.nCastleID = GetCastleID();
	if(Class'Interface.UIPacket'.static.Encode_C_EX_WORLDCASTLEWAR_HOST_CASTLE_SIEGE_RANKING_INFO(stream, packet))
	{
		Debug(("Encode_C_EX_WORLDCASTLEWAR_HOST_CASTLE_SIEGE_RANKING_INFO : " @ string(packet.nCastleID)));
		Class'Interface.UIPacket'.static.RequestUIPacket(743, stream);
	}
	return;
}
...
function API_C_EX_WORLDCASTLEWAR_CASTLE_SIEGE_RANKING_INFO()
{
	...
	packet.nCastleID = GetCastleID();
	if(Class'Interface.UIPacket'.static.Encode_C_EX_WORLDCASTLEWAR_CASTLE_SIEGE_RANKING_INFO(stream, packet))
	{
		Debug(("Encode_C_EX_WORLDCASTLEWAR_CASTLE_SIEGE_RANKING_INFO : " @ string(packet.nCastleID)));
		Class'Interface.UIPacket'.static.RequestUIPacket(744, stream);
	}
	return;
}
...
function int GetCastleID()
{
	...
	castleID = Class'Interface.WorldSiegeWnd'.static.Inst().GetCastleIDSelected();
	if((castleID < 1))
	{
		castleID = 5;
	}
