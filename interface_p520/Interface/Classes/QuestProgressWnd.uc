...
function Initialize()
{
	...
	teleportDialog = Class'Interface.UIControlDialogAssets'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".UIControlDialogAsset")));
...
event OnLoad()
{
	SetClosingOnESC();
	Initialize();
	tObject = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(8000, 1);
	tObject._DelegateOnTime = DlegateOnTimeCheckNew;
	tObjectDeactiveOld = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(4000, 1);
...
event OnRClickListCtrlRecord(string ListCtrlID)
{
	...
	Class'Interface.QuestWnd'.static.Inst()._SetSelectQuest(GetQuestIDAtIndex(Index));
	QuestAlarmList_RichList.SetSelectedIndex(-1, false);
	Class'Interface.QuestWnd'.static.Inst().m_hOwnerWnd.SetFocus();
	return;
}
...
event OnClickListCtrlRecord(string ListCtrlID)
{
	...
	{
		Class'Interface.QuestDialogWnd'.static.Inst()._ShowCompleteDialog(GetQuestIDAtIndex(Index), true);
	}
	else
	{
...
function Handle_S_EX_QUEST_NOTIFICATION(int qid, int nCount, int cNotifType)
{
	...
	switch(cNotifType)
	{
		case 0:
			if(MakeRowData(qid, nCount, false, rowData, oIsComplete))
			{
				...
				Class'Interface.L2Util'.static.Inst().showGfxScreenMessage(((MakeFullSystemMsg(GetSystemMessage(13854), (("&quot;" $ GetQuestName(qid)) $ "&quot;")) $ "<br>") $ currentGFxMsg));
				currentGFxMsg = "";
				_Show(true);
			}
			break;
		...
		case 2:
			...
			Class'Interface.L2Util'.static.Inst().showGfxScreenMessage(((MakeFullSystemMsg(GetSystemMessage(13909), (("&quot;" $ GetQuestName(qid)) $ "&quot;")) $ "<br>") $ currentGFxMsg));
			currentGFxMsg = "";
		case 3:
			if((teleportDialog.nDialogID == qid))
			{
	...
	if((oIsComplete == 1))
	{
		Class'Interface.QuestDialogWnd'.static.Inst()._ShowCompleteDialog(qid);
	}
	if((cNotifType == 0))
	{
...
function RT_S_EX_QUEST_NOTIFICATION()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_QUEST_NOTIFICATION(packet))
	{
		return;
	}
...
function RT_S_EX_QUEST_NOTIFICATION_ALL()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_QUEST_NOTIFICATION_ALL(packet))
	{
		return;
	}
...
{
	Class'Interface.UIPacket'.static.RequestUIPacket(831, _emptyByteArray);
	return;
}
