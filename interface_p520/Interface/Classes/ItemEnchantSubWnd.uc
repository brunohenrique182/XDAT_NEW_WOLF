...
event OnDBClickItem(string ControlName, int Index)
{
	...
	Class'Interface.ItemEnchantWnd'.static.Inst().RequestInputItemInfo(iInfo);
	return;
}
...
function refresh()
{
	switch(Class'Interface.ItemEnchantWnd'.static.Inst().GetStateName())
	{
		case 'stateNone':
			break;
...
function SetSupportItems(ItemWindowHandle supportItemWnd)
{
	...
	if(!Class'Interface.ItemEnchantWnd'.static.Inst().UseSupportSlot())
	{
		return;
	}
...
function ItemInfo GetIteminfoScroll()
{
	...
	Class'Interface.ItemEnchantWnd'.static.Inst().scrollItemWindow.GetItem(0, scrollItem);
	return scrollItem;
}
...
function ItemInfo GetItemInfoEquipment()
{
	...
	Class'Interface.ItemEnchantWnd'.static.Inst().equipmentItemWindow.GetItem(0, equipmentItem);
...
function ItemInfo GetScrollInfoSupport()
{
	...
	Class'Interface.ItemEnchantWnd'.static.Inst().supportItemWindow.GetItem(0, supportiInfo);
	return supportiInfo;
}
