...
{
	uicontrolTextInputScr = Class'Interface.UIControlTextInput'.static.InitScript(GetWindowHandle("DialogBox.DialogBody.DialogEditBox"));
...
function SetWindowStyle(UIScript.EDialogType Style)
{
	...
	switch(Style)
	{
		...
		case DialogType_NumberPadAdena:
			...
			uicontrolTextInputScr._UseCostString(true);
			uicontrolTextInputScr._UseNumericColor(true);
			uicontrolTextInputScr._SetUnderUnit(Class'Interface.WorldExchangeBuyWnd'.static.Inst().ADENA_BASIC_UNIT);
			uicontrolTextInputScr._SetLimitNum(Class'Interface.WorldExchangeBuyWnd'.static.Inst().ADENA_MAX);
			m_cancelHandle.ShowWindow();
			m_NumberPad.ShowWindow();
			SetEditType("number");
			SetDialogWeightBtns();
...
function SetOptionForAdena()
{
	...
	if((Class'Interface.WorldExchangeBuyWnd'.static.Inst()._IsNewServer() == true))
	{
...
function HandleUserOptionAdena(int Option)
{
	...
	if(((Option != 0) && Class'Interface.WorldExchangeBuyWnd'.static.Inst()._IsNewServer()))
	{
		Num = (Num * INT64(10));
	}
	Num = (Num * Class'Interface.WorldExchangeBuyWnd'.static.Inst().ADENA_BASIC_UNIT);
...
function string GetWorldExchangeAdenaZeroString()
{
	...
	else if((Class'Interface.WorldExchangeBuyWnd'.static.Inst()._IsNewServer() == true))
	{
		return "000000";
	}
	return "0000000";
...
function SetTextBoxHeight(string Desc)
{
	...
	Height = Class'Interface.L2Util'.static.Inst()._GetTextBoxHeight(dialogText, Desc);
