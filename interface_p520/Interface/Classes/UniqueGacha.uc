...
function ParsePacket_S_EX_UNIQUE_GACHA_SIDEBAR_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_UNIQUE_GACHA_SIDEBAR_INFO(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_UNIQUE_GACHA_HISTORY()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_UNIQUE_GACHA_HISTORY(packet))
	{
		return;
	}
...
function ShowAllMotionCards()
{
	...
	timeObjectMotion = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(50, gacha_game_packet.ResultItems.Length);
...
function ParsePacket_S_EX_UNIQUE_GACHA_GAME()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_UNIQUE_GACHA_GAME(gacha_game_packet))
	{
		return;
	}
...
function API_C_EX_UNIQUE_GACHA_OPEN(int cFullInfo, int nOpenMode)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_UNIQUE_GACHA_OPEN(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(817, stream);
...
function API_C_EX_UNIQUE_GACHA_GAME(int nGameCount)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_UNIQUE_GACHA_GAME(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(818, stream);
...
function API_C_EX_UNIQUE_GACHA_HISTORY()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_UNIQUE_GACHA_HISTORY(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(821, stream);
	return;
}
...
function addRichListRowDataRewardTotal(int cRankType, int ClassID, INT64 nAmount, float dProb, bool bProb)
{
	...
	if(bProb)
	{
		Class'Interface.L2Util'.static.GetEllipsisString(ItemName, 195);
	}
	else
	{
		Class'Interface.L2Util'.static.GetEllipsisString(ItemName, 240);
	}
