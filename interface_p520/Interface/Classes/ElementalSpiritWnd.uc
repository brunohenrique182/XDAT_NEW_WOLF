...
event OnShow()
{
	...
	Class'Interface.SideBar'.static.Inst().ToggleByWindowName(getCurrentWindowName(string(self)), true);
	return;
}

event OnHide()
{
	Class'Interface.SideBar'.static.Inst().ToggleByWindowName(getCurrentWindowName(string(self)), false);
	super.OnHide();
	return;
}
...
event OnEvent(int a_EventID, string a_Param)
{
	switch(a_EventID)
	{
		case 40:
			bUseSpirit = false;
			Class'Interface.SideBar'.static.Inst().SetWindowShowHideByIndex(1, false);
			break;
		case 10940:
...
function HandleEV_ElementalSpiritSimpleInfo(string a_Param)
{
	...
	Class'Interface.SideBar'.static.Inst().SetAlarmOnOff(1, false);
	i = 0;
	while((i < Len))
	{
	...
	{
		Class'Interface.SideBar'.static.Inst().SetWindowShowHideByIndex(1, getInstanceUIData().GetIsClassicServer());
	}
	i = 0;
	while((i < elementalInfos.Length))
	{
		...
		{
			Class'Interface.SideBar'.static.Inst().SetAlarmOnOff(1, true);
			break;
		}
		i++;
	}
...
function SetElementalExps()
{
	...
	while((i < elementalInfos.Length))
	{
		...
		Info.t_color = Class'Interface.L2Util'.static.Inst().BrightWhite;
		...
		switch((elementalInfos[i].SpiritType - 1))
		{
			case 0:
				...
				Info.t_color = Class'Interface.L2Util'.static.Inst().Yellow;
				Info.bLineBreak = false;
				Info.eAlignType = DIAT_LEFT;
				ToolTip.DrawList[ToolTip.DrawList.Length] = Info;
				Info.t_strText = " / ";
				Info.t_color = Class'Interface.L2Util'.static.Inst().White;
				Info.bLineBreak = false;
				Info.eAlignType = DIAT_LEFT;
				ToolTip.DrawList[ToolTip.DrawList.Length] = Info;
				Info.t_strText = ((GetSystemString(54) $ ":") $ string(UserInfo.nFireDefend));
				Info.t_color = Class'Interface.L2Util'.static.Inst().MintGreen;
				Info.bLineBreak = false;
				Info.eAlignType = DIAT_LEFT;
				ToolTip.DrawList[ToolTip.DrawList.Length] = Info;
				break;
	...
	Class'Interface.SideBar'.static.Inst().GetWindowByIndex(1).SetTooltipCustomType(ToolTip);
	return;
}
...
function RT_S_EX_ELEMENTAL_SPIRIT_ATTACK_TYPE()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_ELEMENTAL_SPIRIT_ATTACK_TYPE(packet))
	{
		return;
	}
