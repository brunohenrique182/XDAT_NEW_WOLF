...
function InitControls()
{
	...
	extractNeedMultiItemScript = Class'Interface.UIControlNeedItemSelectMultiItems'.static._InitScript(GetWindowHandle((extractNeedItemWnd.m_WindowNameWithFullPath $ ".NeedItemMultiItems")));
	...
	noticeDialogAsset = Class'Interface.UIControlDialogAssets'.static.InitScript(GetWindowHandle((noticeDialogWnd.m_WindowNameWithFullPath $ ".NoticePopupAsset")));
	...
	scrollTileList = Class'Interface.UIControlTilelist'.static.InitScript(GetWindowHandle((selectContainerWnd.m_WindowNameWithFullPath $ ".ClassBtnScrollAreaWnd")), 3, 5, true);
	...
	while((i < (3 * 5)))
	{
		...
		slotObject = new Class'Interface.ClassChangeWndSlot';
...
function UpdateSelectInfo()
{
	...
	else if((_uiInfo.Sex == 0))
	{
		...
		maleBtnOverTex.HideWindow();
		btnTwinkleObject._Stop();
		btnTwinkleObject = Class'Interface.L2UITween'.static.Inst()._AddTweenTwinlkle(femaleBtnOverTex, -1.0000000, 0.5000000, 1000.0000000, 0, 220, 0.0000000);
	...
	else if((_uiInfo.Sex == 1))
	{
		...
		btnTwinkleObject._Stop();
		btnTwinkleObject = Class'Interface.L2UITween'.static.Inst()._AddTweenTwinlkle(maleBtnOverTex, -1.0000000, 0.5000000, 1000.0000000, 0, 220, 0.0000000);
		btnTwinkleObject._Play();
	}
	return;
}
...
{
	arrowTwinkleObject = Class'Interface.L2UITween'.static.Inst()._AddTweenTwinlkle(resultArrowTex, -1.0000000, 0.5000000, 2000.0000000, 0, 150, 0.0000000);
...
function Rq_C_EX_CLASS_CHANGE(int ClassID, int raceID, int Sex, int JobGroup, bool bExtractSkill, int extractItemId)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_CLASS_CHANGE(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(922, stream);
	return;
}
...
function Nt_S_EX_CLASS_CHANGE_UI_OPEN()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_CLASS_CHANGE_UI_OPEN(packet))
	{
		return;
	}
	...
	InitTweenObject();
	tabButtons._setTopOrder(0);
	Class'Interface.ClassChangeHistoryWnd'.static.Inst().SetInfo(MakeHistoryClassList());
	SetState(READY);
	Me.ShowWindow();
	return;
}
...
function Rs_S_EX_CLASS_CHANGE_FAIL()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_CLASS_CHANGE_FAIL(packet))
	{
		return;
	}
	return;
...
function Nt_S_EX_ACQUIRE_SKILL_RESULT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_ACQUIRE_SKILL_RESULT(packet))
	{
		return;
	}
	if((packet.cResult == 0))
...
event OnClickButtonWithHandle(ButtonHandle a_ButtonHandle)
{
	...
	{
		Class'Interface.ClassChangeHistoryWnd'.static.Inst().ToggleShow();
	}
	...
	{
		Class'Interface.HelpWnd'.static.ShowHelp(71);
	}
	else
	{
...
{
	ResetInfo();
	Class'Interface.ClassChangeHistoryWnd'.static.Inst().HideInfo();
