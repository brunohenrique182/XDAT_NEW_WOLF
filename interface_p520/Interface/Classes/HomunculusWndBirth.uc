...
function Handle_S_EX_HOMUNCULUS_CREATE_START_RESULT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_HOMUNCULUS_CREATE_START_RESULT(packet))
	{
		return;
	}
	Me.KillTimer(9);
...
function Handle_S_EX_HOMUNCULUS_INSERT_RESULT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_HOMUNCULUS_INSERT_RESULT(packet))
	{
		return;
	}
	AddSystemMessage(packet.nID);
...
function Handle_S_EX_HOMUNCULUS_SUMMON_RESULT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_HOMUNCULUS_SUMMON_RESULT(packet))
	{
		return;
	}
	AddSystemMessage(packet.nID);
