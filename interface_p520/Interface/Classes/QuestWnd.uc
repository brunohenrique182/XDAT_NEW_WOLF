...
event OnTick()
{
	...
	switch(lMouseDownType)
	{
		...
		case showDialogAccecptable:
			...
			Class'Interface.QuestDialogWnd'.static.Inst()._ShowStartDialog(qid);
			break;
		case showDialogAgain:
			Class'Interface.QuestDialogWnd'.static.Inst()._SohwDialogAgain(qid);
			break;
		default:
			break;
	}
...
function Handle_S_EX_QUEST_NOTIFICATION(int qid, int nCount, int cNotifType)
{
	...
	switch(cNotifType)
	{
		...
		case 1:
			...
			if((Class'Interface.QuestDrawerWnd'.static.Inst()._GetCurrentQuestID() == qid))
			{
				Class'Interface.QuestDrawerWnd'.static.Inst()._SetCurrentQuestInfos(qid, nCount, 1);
			}
			break;
		case 2:
			nProceedingQuestCount--;
...
function RT_S_EX_QUEST_UI()
{
	...
	bRequest_C_EX_QUEST_UI = false;
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_QUEST_UI(packet))
	{
		return;
	}
...
function RQ_C_EX_QUEST_UI()
{
	...
	Class'Interface.UIPacket'.static.RequestUIPacket(832, _emptyByteArray);
	return;
}
...
function RT_S_EX_QUEST_ACCEPTABLE_LIST()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_QUEST_ACCEPTABLE_LIST(packet))
	{
		return;
	}
...
function RQ_C_EX_QUEST_ACCEPTABLE_LIST()
{
	...
	Class'Interface.UIPacket'.static.RequestUIPacket(833, _emptyByteArray);
	return;
}
