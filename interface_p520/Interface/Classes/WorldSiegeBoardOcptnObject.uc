...
function _SetMainBattleOccupyInfo(UIPacket._WorldCastleWar_MainBattleOccupyInfo occupyInfo)
{
	...
	Loc = Class'Interface.WorldSiegeBoardWnd'.static.Inst().PositionToVector(occupyInfo.nOccypyNPCLocation);
...
function SetTexture()
{
	if(IsMouseOver)
	{
		Icon_ocptnFlag.SetTexture((("L2UI_EPIC.WorldSiegeWnd.WorldSiegeWnd_Occupation" $ Class'Interface.UICommonAPI'.static.getInstanceUIData().Int2Str(Tier)) $ "_Over"));
	}
	else
	{
		Icon_ocptnFlag.SetTexture((("L2UI_EPIC.WorldSiegeWnd.WorldSiegeWnd_Occupation" $ Class'Interface.UICommonAPI'.static.getInstanceUIData().Int2Str(Tier)) $ "_Normal"));
	}
	return;
}
...
function Color GetStateColor()
{
	switch(GetStateName())
	{
		case 'stateNotOccupied':
			return Class'Interface.L2Util'.static.Inst().White;
		case 'stateAttacked':
			return Class'Interface.L2Util'.static.Inst().Red;
		case 'stateOccupied':
			return Class'Interface.L2Util'.static.Inst().BWhite;
...
function SetCrestTooltipInfo(int PledgeID)
{
	...
	mCustomTooltip.DrawList[Index].t_color = Class'Interface.L2Util'.static.Inst().Gold;
...
state stateOccupiedDespawn
{
	function BeginState()
	{
		...
		Icon_ocptnFlag.SetColorModify(Class'Interface.L2Util'.static.Inst().Gray);
		return;
	}
	...
	function EndState()
	{
		PledgeCrest_tex.HideWindow();
		m_hOwnerWnd.KillTimer(0);
		Icon_ocptnFlag.SetColorModify(Class'Interface.L2Util'.static.Inst().BrightWhite);
		return;
	}
}
