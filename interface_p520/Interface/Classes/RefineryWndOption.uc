...
function SetSimpleProbList(array<string> Options)
{
	...
	while((i < listWnds.Length))
	{
		...
		descHtml.LoadHtmlFromString(ConvertHtmlDesc(Options[i]));
...
function Rq_C_EX_VARIATION_PROB_LIST(int refineryID, int TargetItemID)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_VARIATION_PROB_LIST(stream, packet))
	{
		return;
	}
	Debug((("Rq_C_EX_VARIATION_PROB_LIST" @ string(packet.nRefineryID)) @ string(packet.nTargetItemId)));
	Class'Interface.UIPacket'.static.RequestUIPacket(898, stream);
	return;
}
...
function Rs_S_EX_VARIATION_PROB_LIST()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_VARIATION_PROB_LIST(packet))
	{
		return;
	}
