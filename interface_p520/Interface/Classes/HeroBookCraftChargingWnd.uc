...
function API_C_EX_HERO_BOOK_CHARGE()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_HERO_BOOK_CHARGE(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(805, stream);
	return;
}
...
function API_C_EX_HERO_BOOK_CHARGE_PROB()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_HERO_BOOK_CHARGE_PROB(stream, packet))
	{
		return;
	}
	Log((((" --- C_EX_HERO_BOOK_CHARGE_PROB - nCategory : " $ string(packet.nCategory)) $ " - packet.nTotalItemPoint : ") $ string(packet.nTotalItemPoint)));
	Class'Interface.UIPacket'.static.RequestUIPacket(807, stream);
...
function ParsePacket_S_EX_HERO_BOOK_CHARGE_PROB()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_HERO_BOOK_CHARGE_PROB(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_HERO_BOOK_CHARGE()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_HERO_BOOK_CHARGE(packet))
	{
		return;
	}
	...
	Class'NWindow.UIAPI_TEXTBOX'.static.SetText((m_Windowname $ ".AdenaText"), "0");
	Class'NWindow.UIAPI_TEXTBOX'.static.SetTooltipString((m_Windowname $ ".AdenaText"), "");
	Class'Interface.L2UITimer'.static.Inst()._AddTimerOnce(1, 1)._DelegateOnTime = OnTime;
	return;
}
