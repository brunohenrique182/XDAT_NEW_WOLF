...
function InitControls()
{
	...
	scrollTileList = Class'Interface.UIControlTilelist'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".ScrollAreaWnd")), 4, 1, true);
	...
	while((i < 4))
	{
		...
		itemObject = new Class'Interface.DethroneFireStateItemObject';
...
event OnClickButton(string strID)
{
	...
	{
		case "WindowHelp_BTN":
			Class'Interface.HelpWnd'.static.ShowHelp(63, 6);
			break;
		default:
			break;
	}
...
function Rq_C_EX_HOLY_FIRE_OPEN_UI()
{
	...
	Class'Interface.UIPacket'.static.RequestUIPacket(844, stream);
	return;
}
...
function Rs_S_EX_HOLY_FIRE_OPEN_UI()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_HOLY_FIRE_OPEN_UI(packet))
	{
		return;
	}
