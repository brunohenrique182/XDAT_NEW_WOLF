...
function _Play(optional int _delay)
{
	...
	Paused = false;
	if((Class'Interface.L2UITween'.static.Inst()._GetRotateObjectIndex(Target) < 0))
	{
		Class'Interface.L2UITween'.static.Inst()._AddTweenRotate(Target, int(ease), Angle, Duration, Delay, replay, axisvector);
	}
	return;
}
...
function _Kill()
{
	Class'Interface.L2UITween'.static.Inst()._KillRotateWithWnd(Target);
	return;
}
