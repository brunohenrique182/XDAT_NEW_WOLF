...
function askDialog(int dialogID, optional string mParam)
{
	...
	else if((90021 == dialogID))
	{
		...
		Class'Interface.DialogBox'.static.Inst().setParamInt64(INT64((10 * 1000)));
		Class'Interface.DialogBox'.static.Inst().DialogShow(DialogModalType_Modalless, DialogType_Progress, MakeFullSystemMsg(GetSystemMessage(3690), MentorName, GetClassType(ClassID), string(Level)));
		Class'Interface.DialogBox'.static.Inst().DelegateOnCancel = HandleDialogMenteeCancel;
		Class'Interface.DialogBox'.static.Inst().DelegateOnOK = HandleDialogMenteeOK;
...
function ParsePacket_S_EX_USER_WATCHER_TARGET_STATUS()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_USER_WATCHER_TARGET_STATUS(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_USER_WATCHER_TARGET_LIST()
{
	...
	WatchList.DeleteAllItem();
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_USER_WATCHER_TARGET_LIST(packet))
	{
		return;
	}
	i = 0;
...
function API_C_EX_USER_WATCHER_DELETE(string sTargetName, int nTargetWorldID)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_USER_WATCHER_DELETE(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(701, stream);
...
function API_C_EX_USER_WATCHER_TARGET_LIST()
{
	...
	Class'Interface.UIPacket'.static.RequestUIPacket(699, stream);
