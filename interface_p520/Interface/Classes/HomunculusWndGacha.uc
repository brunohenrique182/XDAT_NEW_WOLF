...
function Handle_S_EX_SHOW_HOMUNCULUS_COUPON_UI()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_SHOW_HOMUNCULUS_COUPON_UI(packet))
	{
		return;
	}
...
function Handle_S_EX_SUMMON_HOMUNCULUS_COUPON_RESULT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_SUMMON_HOMUNCULUS_COUPON_RESULT(packet))
	{
		return;
	}
	if((packet.Type == 1))
...
function API_C_EX_SUMMON_HOMUNCULUS_COUPON(int nItemID)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_SUMMON_HOMUNCULUS_COUPON(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(703, stream);
