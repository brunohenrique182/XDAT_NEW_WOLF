...
function RQ_C_EX_Teleport_UI()
{
	...
	Class'Interface.TeleportWnd'.static.Inst().RQ_C_EX_Teleport_UI();
	return;
}
...
function ParsePacket_S_EX_RAID_TELEPORT_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_RAID_TELEPORT_INFO(packet))
	{
		return;
	}
...
function API_C_EX_RAID_TELEPORT_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_RAID_TELEPORT_INFO(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(617, stream);
	return;
}
...
function API_C_EX_TELEPORT_TO_RAID_POSITION(int nRaidID)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_TELEPORT_TO_RAID_POSITION(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(618, stream);
	return;
}
...
function HandleShowTeleportPopup(string param)
{
	...
	tmpCurrentTelCost = string(Class'Interface.TeleportWnd'.static.Inst().GetTeleportCost(INT64(priceInfo[0]), int(priceInfo[1]), int(priceInfo[2])));
	return;
}
