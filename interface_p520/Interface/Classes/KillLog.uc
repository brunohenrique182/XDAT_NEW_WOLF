...
function SetTweens()
{
	...
	while((i < 7))
	{
		...
		l2UITweenObjectPos[i] = new Class'Interface.L2UITweenObject';
		...
		killLogObjects[i].KillTimer = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject((int(logTime) * 1000));
...
function RT_S_EX_ENEMY_KILL_LOG()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_ENEMY_KILL_LOG(packet))
	{
		return;
	}
...
function _SendToGfxScreenMessageKillLogCenterFadeIn()
{
	...
	if(Class'Interface.PositionManager'.static.Inst()._IsSaved(m_hOwnerWnd.GetWindowName()))
	{
		return;
	}
