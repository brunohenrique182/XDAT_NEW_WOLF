...
function InitControls()
{
	...
	scrollTileList = Class'Interface.UIControlTilelist'.static.InitScript(GetWindowHandle((ownerFullPath $ ".CollectionList_Wnd")), 1, 6, true);
	scrollTileList.DelegateOnItemRenderer = OnChangedTileListRenderer;
	scrollTileList.DelegateOnClickWithHandle = OnClickTileListButton;
	...
	while((i < 6))
	{
		...
		slotObject = new Class'Interface.RelicWndCollectionObject';
		slotObject.Init(itemRendererWnd);
		rendererObjectList[rendererObjectList.Length] = slotObject;
		i++;
	}
	searchTextInput = Class'Interface.UIControlTextInput'.static.InitScript(GetWindowHandle((("RelicWnd." $ searchWnd.m_WindowNameWithFullPath) $ ".TextInput")));
...
function OnClickTileListButton(ButtonHandle btnHandle, int rendererIndex, int Position)
{
	local string btnParentName;
	local int relicIndex;
	local RelicWnd.RelicCollectionInfo CollectionInfo;
	local RelicsCollectionNeed needInfo;

	btnParentName = btnHandle.GetParentWindowName();
	relicIndex = int(Right(btnParentName, 1));
	if((Position < _collectionInfos.Length))
	{
		CollectionInfo = _collectionInfos[Position];
		if((relicIndex < CollectionInfo.Data.NeedRelics.Length))
		{
			needInfo = CollectionInfo.Data.NeedRelics[relicIndex];
			Class'Interface.RelicWnd'.static.Inst().SetUpgradeShortcut(needInfo.RelicsID);
		}
	}
	return;
}
...
function UpdateCollectionList(optional bool needListReset)
{
	...
	_collectionInfos = Class'Interface.RelicWnd'.static.Inst().GetRelicCollectionInfos();
...
function UpdateCollectionCompleteInfo()
{
	...
	collectionInfos = Class'Interface.RelicWnd'.static.Inst().GetRelicCollectionInfos();
...
function bool IsRelicNameMatch(string searchStr, RelicWnd.RelicCollectionInfo CollectionInfo)
{
	...
	while((i < CollectionInfo.Data.NeedRelics.Length))
	{
		...
		relicName = GetItemInfoByClassID(relicUIData.ItemID).Name;
...
function ShowDetailInfoDialog()
{
	...
	while((i < statOptions.Length))
	{
		...
		AddRichListCtrlString(rowData.cellDataList[0].drawitems, Class'Interface.RelicWnd'.static.Inst().GetCollectionStatNameStr(tmpOption), util.ColorGold, false, 10);
		AddRichListCtrlString(rowData.cellDataList[1].drawitems, Class'Interface.RelicWnd'.static.Inst().GetCollectionStatValueStr(tmpOption), util.White, false, 22);
