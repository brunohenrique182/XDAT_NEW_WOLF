...
function Initialize()
{
	...
	NeedItemMultiItems = Class'Interface.UIControlNeedItemSelectMultiItems'.static._InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".UnlockDisableWnd.NeedItem_wnd.NeedItemMultiItems")));
	...
	timeObjectInventoryDelay = Class'Interface.L2UITimer'.static.Inst()._MakeTimerObject(500, 1);
...
function initUIControlNeedItemList()
{
	...
	needItemDialogList = new Class'Interface.UIControlNeedItemList';
	...
	needItemDialogList2 = new Class'Interface.UIControlNeedItemList';
	...
	selectNeedItemList = new Class'Interface.UIControlNeedItemList';
...
event OnClickButton(string Name)
{
	switch(Name)
	{
		...
		case "HelpWnd_Btn":
			Debug("ShowHelp -> 69");
			Class'Interface.HelpWnd'.static.ShowHelp(69);
			break;
		case "HennaClose_BTN":
			CloseUI();
			break;
		...
		case "EnchantbtnHelp":
			Class'Interface.HelpWnd'.static.ShowHelp(69, 3);
			break;
...
function ParsePacket_S_EX_NEW_HENNA_POTEN_OPENSLOT_PROB_INFO()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_NEW_HENNA_POTEN_OPENSLOT_PROB_INFO(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_NEW_HENNA_POTEN_OPENSLOT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_NEW_HENNA_POTEN_OPENSLOT(packet))
	{
		return;
	}
...
function parsepacket_s_ex_new_henna_poten_enchant_reset()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_NEW_HENNA_POTEN_ENCHANT_RESET(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_NEW_HENNA_POTEN_SELECT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_NEW_HENNA_POTEN_SELECT(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_NEW_HENNA_POTEN_ENCHANT()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_NEW_HENNA_POTEN_ENCHANT(packet))
	{
		return;
	}
...
function API_C_EX_NEW_HENNA_POTEN_SELECT(int nSlotID, int nPotenID)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_NEW_HENNA_POTEN_SELECT(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(749, stream);
...
function API_C_EX_NEW_HENNA_POTEN_ENCHANT(int nSlotID, int costItemID)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_NEW_HENNA_POTEN_ENCHANT(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(750, stream);
...
function API_C_EX_NEW_HENNA_POTEN_OPENSLOT(int nSlotID, int nReqOpenSlotStep, int nCostItemClassID)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_NEW_HENNA_POTEN_OPENSLOT(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(906, stream);
...
function API_C_EX_NEW_HENNA_POTEN_OPENSLOT_PROB_INFO(int nReqOpenSlotStep)
{
	...
	packet.nReqOpenSlotStep = nReqOpenSlotStep;
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_NEW_HENNA_POTEN_OPENSLOT_PROB_INFO(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(905, stream);
...
function API_C_EX_NEW_HENNA_POTEN_ENCHANT_RESET(int costItemID)
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_NEW_HENNA_POTEN_ENCHANT_RESET(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(850, stream);
