...
function InitControls()
{
	...
	targetRelicSlot = new Class'Interface.RelicWndSlot';
	targetRelicSlot.Init(relicSlotWnd);
	Class'Interface.RelicWnd'.static.Inst().DelegateChangeUpgradeStuff = OnChangeUpgradeStuff;
	return;
}
...
function UpdateUpgradeInfo()
{
	...
	if(isRegistered)
	{
		...
		statPrevHtml.LoadHtmlFromString(Class'Interface.RelicWnd'.static.Inst().GetHtmlSkillStr(upgradeInfo.RelicInfo.Data.Skills[upgradeInfo.RelicInfo.Level]));
		statNextHtml.LoadHtmlFromString(Class'Interface.RelicWnd'.static.Inst().GetHtmlSkillStr(upgradeInfo.RelicInfo.Data.Skills[(upgradeInfo.RelicInfo.Level + 1)]));
		...
		relicNameTextBox.SetText(GetItemInfoByClassID(upgradeInfo.RelicInfo.Data.ItemID).Name);
...
function ShowResultWnd(bool isShow, bool IsSuccess, optional int Level, optional int relicId)
{
	...
	{
		targetRelicSlot.SetInfo(upgradeInfo.RelicInfo, true);
		sucRelicTextBox.SetText(relicItemInfo.Name);
	...
	else
	{
		...
		failRelicTextBox.SetText(relicItemInfo.Name);
	...
	resultStatHtml.LoadHtmlFromString(Class'Interface.RelicWnd'.static.Inst().GetHtmlSkillStr(upgradeInfo.RelicInfo.Data.Skills[Level]));
	return;
}
...
function ShowUpgradeDialog()
{
	...
	ShowModal();
	Class'Interface.DialogBox'.static.Inst().AnchorToTarget("RelicWnd", 250, 20);
	Class'Interface.DialogBox'.static.Inst().DelegateOnCancel = OnUpgradeDialogCancel;
	Class'Interface.DialogBox'.static.Inst().DelegateOnOK = OnUpgradeDialogConfirm;
	Class'Interface.DialogBox'.static.Inst().DelegateOnHide = OnUpgradeDialogHide;
...
event OnUpgradeDialogConfirm()
{
	if((DialogIsMine() == true))
	{
		Class'Interface.RelicWnd'.static.Inst().RequestRelicUpgrade();
		HideModal();
	}
	return;
}
...
event OnClickButton(string btnName)
{
	switch(btnName)
	{
		case "RelicUpgrade_btn":
			...
			{
				ShowResultWnd(false, false);
				Class'Interface.RelicWnd'.static.Inst().ResetUpgradeStuffList();
			}
			else
			{
				ShowUpgradeDialog();
			...
			break;
		case "RelicResetBtn":
			Class'Interface.RelicWnd'.static.Inst().ResetUpgradeTargetRelic();
