...
function InitControls()
{
	...
	donationDialog = Class'Interface.UIControlDialogAssets'.static.InitScript(GetWindowHandle((modalWnd.m_WindowNameWithFullPath $ ".UIControlDialogAsset")));
...
function UpdatePrisonPanelControls()
{
	...
	inPrisonInfo = Class'Interface.PrisonNoticeHUD'.static.Inst().GetInPrisonInfo();
	prisonNameTextBox.SetText(GetSystemString(Class'Interface.PrisonNoticeHUD'.static.Inst().GetPrisonTitleStringId(prisonData.PrisonType)));
	...
	remainTimeTextBox.SetText(Class'Interface.PrisonNoticeHUD'.static.Inst().GetRemainTimeText(RemainTime));
...
{
	_currentPrisonType = Class'Interface.PrisonNoticeHUD'.static.Inst().GetInPrisonType();
...
function Rq_C_EX_PRISON_USER_DONATION()
{
	...
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_PRISON_USER_DONATION(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(815, stream);
	return;
}
...
function Rs_S_EX_PRISON_USER_DONATION()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_PRISON_USER_DONATION(packet))
	{
		return;
	}
	ClosePrisonWnd();
...
event OnClickButton(string buttonStr)
{
	switch(buttonStr)
	{
		...
		case "Close_btn":
			...
			break;
		case "HelpWnd_Btn":
			Class'Interface.HelpWnd'.static.ShowHelp(65);
			break;
		default:
			break;
	}
