...
function UpdateInventoryWnd()
{
	...
	ItemAutoPeelInfo = Class'Interface.ItemAutoPeelWnd'.static.Inst().GetItemAutoPeelInfo();
...
event OnDBClickItem(string strID, int Index)
{
	...
	if((Index >= 0))
	{
		...
		Class'Interface.ItemAutoPeelWnd'.static.Inst().RegisterItem(targetItemInfo.Id.ServerID, Class'NWindow.InputAPI'.static.IsAltPressed());
	}
	return;
}
