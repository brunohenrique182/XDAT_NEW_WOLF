...
function Initialize()
{
	...
	teleportDialog = Class'Interface.UIControlDialogAssets'.static.InitScript(GetWindowHandle("UIQuestToolWnd.UIControlDialogAsset"));
...
function HandleOnClickContextMenu(int Index)
{
	...
	switch(Index)
	{
		...
		case 3:
			...
			Class'Interface.DialogBox'.static.Inst().DelegateOnOK = HandleCountQuestDataDialogOK;
...
function API_ExecuteCommand(string Command, optional bool bNeedTarget)
{
	...
	if(bNeedTarget)
	{
		...
		{
			Class'Interface.L2Util'.static.Inst().showGfxScreenMessage(GetSystemMessage(50));
			return;
		}
	}
...
function AddItem(int nItemID, int QuestLevel, string fullNameString, string journalName)
{
	...
	Record.LVDataList[1].TextColor = Class'Interface.L2UIColor'.static.Inst().Sandrift;
	...
	{
		Record.LVDataList[3].szData = (("   [" $ fullNameString) $ "]");
		Record.LVDataList[3].TextColor = Class'Interface.L2UIColor'.static.Inst().Sandrift;
	}
	...
	if((clearedQuestID > 0))
	{
		...
		Record.LVDataList[7].TextColor = Class'Interface.L2UIColor'.static.Inst().Sandrift;
...
function AddItemNewQuest(NQuestUIData questUIData)
{
	...
	AddRichListCtrlString(rowData.cellDataList[1].drawitems, GetNewQuestTypeAdena(questUIData.Type), Class'Interface.L2UIColor'.static.Inst().Sandrift, true);
	rowData.cellDataList[2].HiddenStringForSorting = string(questUIData.LevelMin);
	AddRichListCtrlString(rowData.cellDataList[2].drawitems, GetNewLevelString(questUIData), Class'Interface.L2UIColor'.static.Inst().Sandrift, true);
...
function string GetNewQuestType(NQuestUIData questUIData)
{
	...
	return (((((((("<table width=290 border=0 cellpadding=2 cellspacing=2 background=L2UI_CT1.GroupBox.GroupBox_DF><tr><td align=left>" $ questMainType) $ " ") $ htmlfontAdd(GetNewQuestTypeAdena(questUIData.Type), Class'Interface.L2UIColor'.static.Inst()._Color2ZeroX(Class'Interface.L2UIColor'.static.Inst().Sandrift))) $ "</td></tr><tr><td>") $ GetSystemString(922)) $ " ") $ htmlfontAdd(GetNewLevelString(questUIData), Class'Interface.L2UIColor'.static.Inst()._Color2ZeroX(Class'Interface.L2UIColor'.static.Inst().Sandrift))) $ "</td></tr></table>");
}
...
function string GetRewardItems(NQuestRewardData rewardDatas)
{
	...
	NameColor = Class'Interface.L2UIColor'.static.Inst()._Color2ZeroX(Class'Interface.L2UIColor'.static.Inst().BrightWhite);
	numColor = Class'Interface.L2UIColor'.static.Inst()._Color2ZeroX(Class'Interface.L2UIColor'.static.Inst().Sandrift);
	i = 0;
	while((i < Items.Length))
	{
		...
		{
			DisplayString = Class'Interface.L2Util'.static.Inst()._GetItemDisplayString(Class'Interface.L2Util'.static.Inst()._GetItemDisplayType(Items[i].ItemClassID), Items[i].Amount);
...
function string HtmlTitleTableAdd(string Title, optional Color C)
{
	...
	{
		C = Class'Interface.L2UIColor'.static.Inst().Sandrift;
	}
	hex = Class'Interface.L2UIColor'.static.Inst()._Color2ZeroX(C);
