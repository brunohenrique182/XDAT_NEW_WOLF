...
function UpdateInfo()
{
	...
	{
		Grade = Class'Interface.RelicWndList'.static.Inst().GetSelectedTabGrade();
	}
	if((int(Grade) == 0))
	{
		_grade = RG_NONE;
	...
	if((successProbList.Length > 0))
	{
		...
		while((i < (successProbList.Length + 1)))
		{
			...
			if((i == 0))
			{
				...
				AddRichListCtrlString(rowData.cellDataList[0].drawitems, Class'Interface.RelicWnd'.static.Inst().GetRelicGradeString(successGrade), util.BWhite, false, 8, , "hs12");
			}
			else
			{
				rowData.sOverlayTex = "";
				tmpInfo = successProbList[(i - 1)];
				AddRichListCtrlString(rowData.cellDataList[0].drawitems, tmpInfo.relicName, util.GetRelicTextColor(successGrade), false, 10);
	...
	if((failProbList.Length > 0))
	{
		...
		while((i < (failProbList.Length + 1)))
		{
			...
			if((i == 0))
			{
				...
				rowData.OverlayTexU = 334;
				rowData.OverlayTexV = 42;
				AddRichListCtrlString(rowData.cellDataList[0].drawitems, Class'Interface.RelicWnd'.static.Inst().GetRelicGradeString(failGrade), util.BWhite, false, 8, , "hs12");
			}
			else
			{
				rowData.sOverlayTex = "";
				tmpInfo = failProbList[(i - 1)];
				AddRichListCtrlString(rowData.cellDataList[0].drawitems, tmpInfo.relicName, util.GetRelicTextColor(failGrade), false, 10);
				AddRichListCtrlString(rowData.cellDataList[1].drawitems, util.MakeDecimalPointString(string(tmpInfo.Prob), 8, true, true), util.White, false, 22);
			}
			RichListCtrl.InsertRecord(rowData);
			i++;
...
function bool IsGradeSame(int newType)
{
	if((newType == 1))
	{
		return (int(_grade) == int(Class'Interface.RelicWnd'.static.Inst().GetCombineInfo().Grade));
	}
	else if((newType == 3))
	{
		return (int(_grade) == int(Class'Interface.RelicWndList'.static.Inst().GetSelectedTabGrade()));
	}
	return false;
}
