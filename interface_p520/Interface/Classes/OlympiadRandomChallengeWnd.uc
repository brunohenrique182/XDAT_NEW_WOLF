...
function API_C_EX_OLYMPIAD_MATCH_MAKING()
{
	...
	packet.cGameRuleType = 0;
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_OLYMPIAD_MATCH_MAKING(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(585, stream);
...
function API_C_EX_OLYMPIAD_MATCH_MAKING_CANCEL()
{
	...
	packet.cGameRuleType = 0;
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_OLYMPIAD_MATCH_MAKING_CANCEL(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(586, stream);
