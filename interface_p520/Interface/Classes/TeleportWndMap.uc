...
function ShowMapIconTooltip(int TeleportID)
{
	...
	TeleportInfo = Class'Interface.TeleportWnd'.static.Inst().GetTeleportInfo(TeleportID, true);
	tooltipRect = mapTooltip.GetRect();
	CostText = MakeCostStringINT64(Class'Interface.TeleportWnd'.static.Inst().GetTeleportCost(TeleportInfo.Price[0].Amount, TeleportInfo.UsableLevel, TeleportInfo.UsableTransferDegree));
...
function UpdateTownZoneIcons(bool isRecommentTypw)
{
	...
	{
		townInfoList = Class'Interface.TeleportWnd'.static.Inst().GetTeleportTownList();
	}
	i = 0;
	while((i < townMapIcons.Length))
	{
...
function UpdateDominionZoneIcons()
{
	...
	selectedTownId = Class'Interface.TeleportWnd'.static.Inst().GetSelectedTownID();
	selectedRcZoneId = Class'Interface.TeleportWnd'.static.Inst().GetSelectedRcZoneID();
	selectedDominionTeleportId = Class'Interface.TeleportWnd'.static.Inst().GetSelectedDominionTeleportID();
	if((selectedRcZoneId > 0))
	{
		townInfo = Class'Interface.TeleportWnd'.static.Inst().GetRcTownInfo(selectedRcZoneId);
		townAreaAnimTex.HideWindow();
		townIconAnimTex.HideWindow();
	}
	else if((selectedTownId > 0))
	{
		townInfo = Class'Interface.TeleportWnd'.static.Inst().GetTownInfo(selectedTownId);
...
event OnTimer(int TimerID)
{
	...
	{
		UpdateCurrentZoneInfo();
		if(Class'Interface.TeleportWnd'.static.Inst().Me.IsShowWindow())
		{
...
event OnClickButton(string Name)
{
	...
	{
		case "MyLocation_Btn":
			Class'Interface.TeleportWnd'.static.Inst().SetCurrentZoneListSelected(true);
			break;
		default:
			break;
	}
...
event OnClickButtonWithHandle(ButtonHandle a_ButtonHandle)
{
	...
	if((buttonValue > 0))
	{
		Class'Interface.TeleportWnd'.static.Inst().ShowTeleportDialog(INT64(buttonValue));
	}
	return;
}
