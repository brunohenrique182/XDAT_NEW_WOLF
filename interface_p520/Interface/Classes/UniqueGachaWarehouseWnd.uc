...
function MoveItemTopToBottom(int Index, bool bAllItem)
{
	...
	if(m_topList.GetItem(Index, topInfo))
	{
		if(((!bAllItem && IsStackableItem(topInfo.ConsumeType)) && (topInfo.ItemNum > INT64(1))))
		{
			...
			Class'Interface.DialogBox'.static.Inst().DelegateOnHide = DelegateOnHide;
		}
		else
		{
...
function API_C_EX_UNIQUE_GACHA_INVEN_GET_ITEM()
{
	...
	if((m_bottomList.GetItemNum() > 0))
	{
		...
		if(!Class'Interface.UIPacket'.static.Encode_C_EX_UNIQUE_GACHA_INVEN_GET_ITEM(stream, packet))
		{
			return;
		}
		Class'Interface.UIPacket'.static.RequestUIPacket(820, stream);
...
function API_C_EX_UNIQUE_GACHA_INVEN_ITEM_LIST()
{
	...
	packet.cInvenType = 0;
	if(!Class'Interface.UIPacket'.static.Encode_C_EX_UNIQUE_GACHA_INVEN_ITEM_LIST(stream, packet))
	{
		return;
	}
	Class'Interface.UIPacket'.static.RequestUIPacket(819, stream);
...
function ParsePacket_S_EX_UNIQUE_GACHA_INVEN_GET_ITEM()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_UNIQUE_GACHA_INVEN_GET_ITEM(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_UNIQUE_GACHA_INVEN_ITEM_LIST()
{
	...
	if(!Class'Interface.UIPacket'.static.Decode_S_EX_UNIQUE_GACHA_INVEN_ITEM_LIST(packet))
	{
		return;
	}
