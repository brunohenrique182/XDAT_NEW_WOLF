...
event OnLoad()
{
	...
	tObjectTalk = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(50, -1);
	tObjectTalk._DelegateOnStart = HandleTimerOnStart;
	tObjectTalk._DelegateOnTime = HandleTimerOnTime;
	tObjectDist = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(50, -1);
...
function ShowCurrentHelp()
{
	...
	{
		Class'InterfaceClassic.HelpWnd'.static.ShowHelp(currentHelpIndex);
	}
	return;
}
...
function HandleConfirmBtn()
{
	switch(_currentDialogType)
	{
		case 1:
			...
			break;
		case 2:
			Class'InterfaceClassic.QuestConfirmWnd'.static.Inst()._ShowQuestConfirm();
			break;
		case 3:
			ShowTeleport();
			break;
		case 4:
			Class'InterfaceClassic.QuestConfirmWnd'.static.Inst()._ShowQuestEnd();
			break;
		default:
			break;
	}
...
event OnHide()
{
	...
	textBoxList.Clear();
	dialog = "";
	Class'InterfaceClassic.QuestConfirmWnd'.static.Inst().m_hOwnerWnd.HideWindow();
	if(DialogIsMine())
	{
...
event OnSetFocus(WindowHandle wndHandle, bool bFocused)
{
	...
	if(bFocused)
	{
		Class'InterfaceClassic.QuestConfirmWnd'.static.Inst().m_hOwnerWnd.BringToFront();
	}
	return;
}
...
function RT_S_EX_QUEST_DIALOG()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_QUEST_DIALOG(packet))
	{
		return;
	}
...
function RQ_C_EX_QUEST_ACCEPT(optional bool bAccept)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_QUEST_ACCEPT(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(828, stream);
	return;
}
...
function RQ_C_EX_QUEST_COMPLETE()
{
	...
	packet.nID = _currentQuestID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_QUEST_COMPLETE(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(830, stream);
	return;
}
