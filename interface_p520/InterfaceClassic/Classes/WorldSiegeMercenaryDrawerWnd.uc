...
function Handle_S_EX_WORLDCASTLEWAR_PLEDGE_MERCENARY_MEMBER_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_WORLDCASTLEWAR_PLEDGE_MERCENARY_MEMBER_LIST(packet))
	{
		return;
	}
	...
	Class'InterfaceClassic.WorldSiegeMercenaryWnd'.static.Inst().ModifyMercenaryMemberCount(SiegeMercenaryEntry_List.GetRecordCount(), packet.nPledgeSId);
	return;
}
...
function Handle_S_EX_WORLDCASTLEWAR_PLEDGE_MERCENARY_MEMBER_JOIN()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_WORLDCASTLEWAR_PLEDGE_MERCENARY_MEMBER_JOIN(packet))
	{
		return;
	}
	if((packet.nResult == 0))
...
function API_C_EX_WORLDCASTLEWAR_PLEDGE_MERCENARY_MEMBER_LIST()
{
	...
	if(Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_WORLDCASTLEWAR_PLEDGE_MERCENARY_MEMBER_LIST(stream, packet))
	{
		Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(737, stream);
	}
	return;
}
...
function API_C_EX_WORLDCASTLEWAR_PLEDGE_MERCENARY_MEMBER_JOIN()
{
	...
	if(Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_WORLDCASTLEWAR_PLEDGE_MERCENARY_MEMBER_JOIN(stream, packet))
	{
		Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(738, stream);
	}
	return;
}
