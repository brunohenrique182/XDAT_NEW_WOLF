...
{
	tObject = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(900, 1);
...
function _SetRemainTime(int RemainTime)
{
	...
	{
		Time_Txt.SetText(Class'InterfaceClassic.L2Util'.static.Inst().TimeNumberToString(RemainTime));
	}
	return;
}
...
{
	Class'InterfaceClassic.L2UITween'.static.Inst()._AddTweenTwinlkle(attendHighlight, 3.5000000, 0.5000000, 1000.0000000);
	return;
}

function UnSetHighLight()
{
	Class'InterfaceClassic.L2UITween'.static.Inst()._KillTwinkleWithWnd(attendHighlight);
	attendHighlight.SetAlpha(0);
	return;
}
