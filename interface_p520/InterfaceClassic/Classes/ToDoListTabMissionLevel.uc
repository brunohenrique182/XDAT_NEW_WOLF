...
function Initialize()
{
	...
	levelJumpDialogAsset = Class'InterfaceClassic.UIControlDialogAssets'.static.InitScript(levelJumpDialogWnd);
...
function Rq_C_EX_MISSION_LEVEL_REWARD_LIST()
{
	...
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(776, stream);
	return;
}
...
function Rq_C_EX_MISSION_LEVEL_JUMP_LEVEL()
{
	...
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(778, stream);
	return;
}
...
function Rq_C_EX_MISSION_LEVEL_RECEIVE_REWARD(int Level, EMissionLevelRewardType RewardType)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_MISSION_LEVEL_RECEIVE_REWARD(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(777, stream);
	return;
}
...
function Rs_S_EX_MISSION_LEVEL_REWARD_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_MISSION_LEVEL_REWARD_LIST(packet))
	{
		return;
	}
