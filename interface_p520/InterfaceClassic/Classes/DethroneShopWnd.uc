...
event OnShow()
{
	...
	Class'InterfaceClassic.L2UITimer'.static.Inst()._AddTimerOnce(100)._DelegateOnTime = OnTime;
	return;
}
...
function Initialize()
{
	...
	InitNeedItem();
	InitInputControl();
	delayTimeObject = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(1, 1);
...
function HandleS_EX_DETHRONE_SHOP_BUY()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_DETHRONE_SHOP_BUY(packet))
	{
		return;
	}
...
function HandleS_EX_DETHRONE_POINT_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_DETHRONE_POINT_INFO(packet))
	{
		return;
	}
...
function API_C_EX_DETHRONE_SHOP_BUY(int ClassID, INT64 nCount)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_DETHRONE_SHOP_BUY(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(839, stream);
...
{
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(838, _emptyByteArray);
