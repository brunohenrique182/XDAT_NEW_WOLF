...
event OnLoad()
{
	...
	timerObject = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject();
	SetGrouIDWnd();
...
function SetLoadEnchantEffectItem_Wnd()
{
	...
	tObjectEnchantEffectItem_Wnd_Show = new Class'InterfaceClassic.L2UITweenObject';
	tObjectEnchantEffectItem_Wnd_Hide = new Class'InterfaceClassic.L2UITweenObject';
...
function HandleShowDialogAuto()
{
	...
	Class'InterfaceClassic.DialogBox'.static.Inst().DelegateOnCancel = DialogResultCancel;
	Class'InterfaceClassic.DialogBox'.static.Inst().DelegateOnOK = DialogResultOKAuto;
	Class'InterfaceClassic.DialogBox'.static.Inst().SetDefaultAction(EDefaultCancel);
	Class'InterfaceClassic.ItemEnchantSubWnd'.static.Inst().m_hOwnerWnd.HideWindow();
	return;
}
...
function HandleShowDialog()
{
	...
	Class'InterfaceClassic.DialogBox'.static.Inst().AnchorToOwner(0, 200);
	Class'InterfaceClassic.DialogBox'.static.Inst().DelegateOnCancel = DialogResultCancel;
	Class'InterfaceClassic.DialogBox'.static.Inst().DelegateOnOK = DialogResultOK;
	Class'InterfaceClassic.DialogBox'.static.Inst().SetDefaultAction(EDefaultCancel);
	Class'InterfaceClassic.ItemEnchantSubWnd'.static.Inst().m_hOwnerWnd.HideWindow();
	return;
}
...
function DialogResultCancel()
{
	...
	EnchantBtnAuto.EnableWindow();
	isAutoEnchanting = false;
	Class'InterfaceClassic.ItemEnchantSubWnd'.static.Inst().m_hOwnerWnd.ShowWindow();
	return;
}
...
function HandleEnchantShow(string param)
{
	...
	if(!Class'InterfaceClassic.L2UIInventory'.static.Inst().FindItem(cID, iInfos))
	{
		return;
	}
...
function Handle_S_EX_RESET_ENCHANT_CHALLENGE_POINT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_RESET_ENCHANT_CHALLENGE_POINT(packet))
	{
		return;
	}
...
function Handle_S_EX_SET_ENCHANT_CHALLENGE_POINT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SET_ENCHANT_CHALLENGE_POINT(packet))
	{
		return;
	}
	EnchantBtn.EnableWindow();
...
function Handle_S_EX_RES_ENCHANT_ITEM_FAIL_REWARD_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_RES_ENCHANT_ITEM_FAIL_REWARD_INFO(packet))
	{
		return;
	}
...
function Handle_S_EX_CHANGED_ENCHANT_TARGET_ITEM_PROB_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_CHANGED_ENCHANT_TARGET_ITEM_PROB_LIST(packet))
	{
		return;
	}
...
function SetSupportItemList()
{
	if(supportItemListWnd.IsShowWindow())
	{
		Class'InterfaceClassic.ItemEnchantSubWnd'.static.Inst().SetSupportItems(supportItemWnds);
	}
	return;
}
...
function SetEnchantEffectItemTitle()
{
	...
	if(isShow)
	{
		...
		GetTextBoxHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".EnchantEffectBTN_Text")).SetTextColor(Class'InterfaceClassic.L2UIColor'.static.Inst().White);
	...
	else
	{
		...
		GetTextBoxHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".EnchantEffectBTN_Text")).SetTextColor(Class'InterfaceClassic.L2UIColor'.static.Inst().Gray);
...
function SetEnchantEffectItemAfter(int EnchantedNum)
{
	...
	switch(byte(GetIteminfoScroll().EtcItemType))
	{
		...
		case 87:
			...
			{
				GetTextBoxHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".EnchantEffectItem_Wnd.CurrentEffectTitle_Ctrl01")).SetTextColor(Class'InterfaceClassic.L2UIColor'.static.Inst().Gray);
			}
			break;
		default:
			if((((enchantedBefore + GetRandomValue()) >= EnchantedNum) && (EnchantedNum > enchantedBefore)))
...
function SetFailItems(int GroupID, int point, array<UIPacket._EnchantRewardItem> rewardItemLits)
{
	...
	if((point > 0))
	{
		...
		Class'NWindow.UIDATA_ITEM'.static.GetItemInfo(GetItemID(Class'InterfaceClassic.ItemEnchantGroupIDWnd'.static._GetClassIDPoint(GroupID)), iInfo);
		...
		Class'InterfaceClassic.L2Util'.static.SetEllipsisTextBox(GetTextBoxHandle((CurrentPath $ "FailItemName_txt")));
	...
	while((i < rewardItemLits.Length))
	{
		...
		GetItemWindowHandle((CurrentPath $ "FailItem_ItemWnd")).AddItem(iInfo);
		GetTextBoxHandle((CurrentPath $ "FailItemName_txt")).SetText(iInfo.Name);
		GetTextBoxHandle((CurrentPath $ "FailItemName_txt")).SetTextColor(Class'InterfaceClassic.L2UIColor'.static.Inst().White);
		Class'InterfaceClassic.L2Util'.static.SetEllipsisTextBox(GetTextBoxHandle((CurrentPath $ "FailItemName_txt")));
		GetTextBoxHandle((CurrentPath $ "FailItemNum_txt")).SetTextColor(Class'InterfaceClassic.L2UIColor'.static.Inst().Gold);
...
function AddFailChallangePointInfo()
{
	...
	{
		Class'NWindow.UIDATA_ITEM'.static.GetItemInfo(GetItemID(Class'InterfaceClassic.ItemEnchantGroupIDWnd'.static._GetClassIDPoint(failChallangePointInfo.gropuID)), iInfo);
...
function CheckEmptyFailItemInfoToList()
{
	...
	AddRichListCtrlString(rowData.cellDataList[0].drawitems, GetSystemString(14024), Class'InterfaceClassic.L2Util'.static.Inst().Gray, false, 10);
...
function bool MakeRecordFailItem(ItemInfo iInfo, out RichListCtrlRowData rowData)
{
	...
	AddRichListCtrlString(rowData.cellDataList[0].drawitems, GetItemNameAll(iInfo), Class'InterfaceClassic.L2Util'.static.Inst().White, false, 4, 3);
	AddRichListCtrlString(rowData.cellDataList[0].drawitems, ("x" $ MakeCostStringINT64(iInfo.ItemNum)), Class'InterfaceClassic.L2Util'.static.Inst().White, true, 42, 0);
	return true;
}
...
function API_C_EX_SET_ENCHANT_CHALLENGE_POINT(bool useTicket)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SET_ENCHANT_CHALLENGE_POINT(stream, packet))
	{
		return;
	}
	API_RequestExRemoveEnchantSupportItem();
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(762, stream);
	return;
}
...
function API_C_EX_RESET_ENCHANT_CHALLENGE_POINT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_RESET_ENCHANT_CHALLENGE_POINT(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(763, stream);
	return;
}
...
function API_C_EX_REQ_ENCHANT_FAIL_REWARD_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_REQ_ENCHANT_FAIL_REWARD_INFO(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(761, stream);
	return;
}
...
function API_C_EX_REQ_VIEW_ENCHANT_RESULT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_REQ_VIEW_ENCHANT_RESULT(stream, packet))
	{
		return;
	}
	NoticeWnd(GetScript("NoticeWnd"))._CreateCollectionButtonBlind();
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(764, stream);
	return;
}
...
function SwapMultiEnchantWnd()
{
	...
	Class'InterfaceClassic.ItemMultiEnchantWnd'.static.Inst()._HandleOnDrop(GetIteminfoScroll());
	m_hOwnerWnd.HideWindow();
	return;
}
...
{
	return Class'InterfaceClassic.L2UIColor'.static.Inst()._Color2ZeroX(Class'InterfaceClassic.L2UIColor'.static.Inst().Gray);
	switch(nFontLevel)
	{
		case 1:
			return "ffe57f";
...
function string AgathionSkillInfoToHtml(ItemInfo Info)
{
	...
	{
		titleColor = Class'InterfaceClassic.L2UIColor'.static.Inst().Red3;
		DescColor = Class'InterfaceClassic.L2UIColor'.static.Inst().ColorGray;
...
function InputScrollItem()
{
	...
	API_RequestExRemoveEnchantSupportItem();
	GotoState('stateReadyEquipment');
	Class'InterfaceClassic.ItemEnchantSubWnd'.static.Inst().refresh();
