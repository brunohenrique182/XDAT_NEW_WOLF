...
{
	SetClosingOnESC();
	tObject = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(2000);
...
function RQ_C_EX_WORLD_EXCHANGE_SETTLE_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_WORLD_EXCHANGE_SETTLE_LIST(stream, packet))
	{
		return;
	}
	b_DisableWindow = true;
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(787, stream);
	return;
}
...
function RT_S_EX_WORLD_EXCHANGE_SETTLE_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_WORLD_EXCHANGE_SETTLE_LIST(packet))
	{
		return;
	}
...
function RT_S_EX_WORLD_EXCHANGE_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_WORLD_EXCHANGE_INFO(packet))
	{
		return;
	}
	Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst()._SetMaxRegiItemNum(packet.nMaxSlot);
...
function handleResult()
{
	...
	Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst()._SetCurrentRigedItemNum(_GetRegiItemCount());
	if((receivedNum == 0))
	{
...
function RQ_C_EX_WORLD_EXCHANGE_SETTLE_RECV_RESULT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_WORLD_EXCHANGE_SETTLE_RECV_RESULT(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(788, stream);
	return;
}
...
function RT_S_EX_WORLD_EXCHANGE_SETTLE_RECV_RESULT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_WORLD_EXCHANGE_SETTLE_RECV_RESULT(packet))
	{
		return;
	}
...
{
	b_DisableWindow = true;
	Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst()._ShowDisableWIndow();
	tObject._Reset();
	return;
}
...
{
	b_DisableWindow = false;
	Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst()._HideDisableWindow();
	return;
}
...
{
	Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst()._SetShowCancelDialog(itemReservedString);
	return;
}
...
function bool MakeRowDataAdena(UIPacket._WorldExchangeItemData _itemData, out RichListCtrlRowData outRowData, listType _listType)
{
	...
	RemainTime = ((_itemData.nExpiredTime - Class'InterfaceClassic.UIData'.static.Inst().serverStartTime) - Class'InterfaceClassic.UIData'.static.Inst().gameConnectTimeSec());
	...
	AddRichListCtrlString(rowData.cellDataList[0].drawitems, Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst()._MakeAdenaString(iInfo.ItemNum), GetNumericColor(string(iInfo.ItemNum)), false, 4, 9);
	...
	switch(_listType)
	{
		case Wait:
			...
			break;
		case Normal:
			AddRichListCtrlString(rowData.cellDataList[2].drawitems, Class'InterfaceClassic.L2Util'.static.Inst().getTimeStringBySec3(RemainTime), GTColor().White, false);
			break;
		default:
			AddRichListCtrlString(rowData.cellDataList[2].drawitems, Class'InterfaceClassic.L2Util'.static.Inst().getTimeStringBySec3(RemainTime), GTColor().Red, false);
			break;
	}
	...
	strs = Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst()._GetDividedValue2Strs(iInfo.Price, (iInfo.ItemNum / Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ADENA_BASIC_UNIT), 3);
	AddRichListCtrlString(rowData.cellDataList[4].drawitems, "", GetColor(0, 0, 0, 0), false, 0, 0);
	AddRichListCtrlString(rowData.cellDataList[4].drawitems, ("." $ strs[1]), GetColor(123, 123, 123, 255), false, 0, 1, "hs7");
	AddRichListCtrlString(rowData.cellDataList[4].drawitems, strs[0], GetNumericColor(strs[0]), false, 0, -2);
...
function INT64 GetCommitionSell(INT64 Cnt)
{
	...
	tmpWorldExchangeUIData = Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst().API_GetWorldExchangeData();
	return INT64(Min(tmpWorldExchangeUIData.MaxSellFee, int((float(Cnt) * (float(tmpWorldExchangeUIData.SellFee) / 100.0000000)))));
}
