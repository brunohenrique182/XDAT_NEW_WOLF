...
event OnSetFocus(WindowHandle wndHandle, bool bFocused)
{
	if(bFocused)
	{
		Class'InterfaceClassic.QuestDialogWnd'.static.Inst().m_hOwnerWnd.SetFocus();
	}
	return;
}
...
{
	m_hOwnerWnd.HideWindow();
	Class'InterfaceClassic.QuestDialogWnd'.static.Inst()._ConfirmQuest();
	return;
}
...
function SetCurrentQuestInfos(bool bConfirm)
{
	...
	questDialogWndScr = Class'InterfaceClassic.QuestDialogWnd'.static.Inst();
...
function SetTxtQuestTargetText(string Goalname, int goalNum, bool bConfirm)
{
	...
	if((nWidth > 272))
	{
		...
		Class'InterfaceClassic.L2Util'.static.GetEllipsisString(Goalname, (272 - nWidth));
	}
	else
	{
