...
function InitLCointCraftBtnSize()
{
	...
	if((strWidth > 50))
	{
		...
		Class'InterfaceClassic.L2Util'.static.GetEllipsisString(labelStr, 100);
...
function UpdateAdenaControls()
{
	...
	else
	{
		...
		else
		{
			...
			Class'InterfaceClassic.L2Util'.static.GetEllipsisString(Adenastring, 105);
...
function Nt_S_EX_MAGICLAMP_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_MAGICLAMP_INFO(packet))
	{
		return;
	}
...
function Nt_S_EX_SUBJUGATION_SIDEBAR()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SUBJUGATION_SIDEBAR(packet))
	{
		return;
	}
