...
function OnDialogOK()
{
	...
	{
		API_C_EX_TIME_RESTRICT_FIELD_HOST_USER_LEAVE(NoticeHUD(GetScript("NoticeHud")).getTimeZoneCurrentFieldID(), Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerFieldID);
	}
	else
	{
		Class'InterfaceClassic.SkyTowerWnd'.static.Inst().API_C_EX_SERVERWAR_MOVE_TO_HOST(Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerFieldID);
	}
	GetPopupExpandScript().Hide();
	showDisable(false);
	Me.HideWindow();
...
{
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SERVERWAR_FIELD_ENTER_USER_INFO(fieldEnterPacket))
	{
		return;
	}
...
function API_C_EX_SERVERWAR_FIELD_ENTER_USER_INFO(int nFieldID)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SERVERWAR_FIELD_ENTER_USER_INFO(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(864, stream);
