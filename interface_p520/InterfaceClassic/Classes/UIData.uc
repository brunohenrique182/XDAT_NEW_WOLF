...
event OnLoad()
{
	...
	MAXLV = GetMaxLevel();
	SetBuilderPC();
	timer24Object = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(((((1000 * 60) * 60) * 12) + Rand((1000 * 60))), -1);
...
function ParsePacket_S_EX_CRAFT_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_CRAFT_INFO(packet))
	{
		return;
	}
...
function RT_S_EX_ADENLAB_TOOLTIP_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_ADENLAB_TOOLTIP_INFO(packet))
	{
		return;
	}
