...
function Initialize()
{
	scrollTesterV = Class'InterfaceClassic.UIControlTilelistScroll'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".ScrollAreaWndV")), 3, 7, true, "itemRenderer");
...
function RT_S_EX_CHARACTER_STYLE_SELECT()
{
	...
	twinkleObjectSkillSelected = Class'InterfaceClassic.L2UITween'.static.Inst()._AddTweenTwinlkle(skillActivate_tex, 1.5000000, 0.5000000, 800.0000000, 0, 255, 0.0000000);
	return;
}
...
function RQ_C_EX_CHARACTER_STYLE_UPDATE_FAVORITE(int StyleID, bool onOff)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_CHARACTER_STYLE_UPDATE_FAVORITE(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(929, stream);
	return;
}
...
function RT_S_EX_CHARACTER_STYLE_UPDATE_FAVORITE()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_CHARACTER_STYLE_UPDATE_FAVORITE(packet))
	{
		return;
	}
	if((packet.cResult == 0))
