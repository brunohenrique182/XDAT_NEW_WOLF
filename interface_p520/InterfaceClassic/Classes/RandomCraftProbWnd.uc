...
function RichListCtrlRowData MakeRowData(int Index)
{
	...
	probString = Class'InterfaceClassic.L2Util'.static.Inst().MakeDecimalPointString(string(probInfos[currentSlot].craftItemInfos[Index].nProb), 6, true, true);
...
function RQ_C_EX_CRAFT_SLOT_PROB_LIST(int SlotIndex)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_CRAFT_SLOT_PROB_LIST(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(896, stream);
	return;
}
...
function RT_S_EX_CRAFT_SLOT_PROB_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_CRAFT_SLOT_PROB_LIST(packet))
	{
		return;
	}
