...
function OnClickButton(string Name)
{
	...
	switch(Name)
	{
		...
		case "VoteRichlistBtn":
			if((Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerCurrentState == 2))
			{
...
{
	if((Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerCurrentState == 4))
	{
		GotoState('cOngoing');
	}
	else
...
{
	API_C_EX_SERVERWAR_GET_REWARD(Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerFieldID);
	return;
}
...
{
	Class'InterfaceClassic.HelpWnd'.static.ShowHelp(74);
	return;
}
...
{
	API_C_EX_SERVERWAR_BATTLE_HUD_INFO(Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerFieldID);
	return;
}
...
function OnTeleport_btnClick()
{
	...
	API_C_EX_SERVERWAR_MOVE_TO_LEADER_CAMP(Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerFieldID, nSelectLeaderWorldID, nSelectLeaderDBID);
	Me.HideWindow();
	return;
}
...
function ParsePacket_S_EX_SERVERWAR_NOTIFY_SET_LEADER()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SERVERWAR_NOTIFY_SET_LEADER(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_SERVERWAR_FIELD_ENTER_HUD_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SERVERWAR_FIELD_ENTER_HUD_INFO(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_SERVERWAR_GET_REWARD()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SERVERWAR_GET_REWARD(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_SERVERWAR_REWARD_ITEM_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SERVERWAR_REWARD_ITEM_INFO(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_SERVERWAR_REWARD_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SERVERWAR_REWARD_INFO(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_SERVERWAR_SELECT_LEADER()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SERVERWAR_SELECT_LEADER(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_SERVERWAR_SELECT_LEADER_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SERVERWAR_SELECT_LEADER_INFO(packet))
	{
		return;
	}
	...
	nSelectLeaderWorldID = packet.nLeaderWorldID;
	nSelectLeaderDBID = packet.nLeaderDBID;
	if(((Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerCurrentState == 4) || (Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerCurrentState == 5)))
	{
	}
	else
	{
...
function ParsePacket_S_EX_SERVERWAR_BATTLE_HUD_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SERVERWAR_BATTLE_HUD_INFO(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_SERVERWAR_LEADER_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SERVERWAR_LEADER_LIST(packet))
	{
		return;
	}
	...
	// lstServerWarLeaderInfoList.Sort(SortByPoint);   // array.Sort() unsupported by this compiler
	if(((Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerCurrentState == 5) || (Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerCurrentState == 3)))
	{
	}
	else if((Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerCurrentState == 4))
	{
		Me.ShowWindow();
		GotoState('None');
		GotoState('cOngoing');
...
function API_C_EX_SERVERWAR_MOVE_TO_HOST(int nFieldID)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SERVERWAR_MOVE_TO_HOST(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(865, stream);
...
function API_C_EX_SERVERWAR_MOVE_TO_LEADER_CAMP(int nFieldID, int nLeaderWorldID, int nLeaderDBID)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SERVERWAR_MOVE_TO_LEADER_CAMP(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(870, stream);
...
function API_C_EX_SERVERWAR_BATTLE_HUD_INFO(int nFieldID)
{
	...
	packet.nFieldID = nFieldID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SERVERWAR_BATTLE_HUD_INFO(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(866, stream);
...
function API_C_EX_SERVERWAR_LEADER_LIST(int nFieldID)
{
	...
	packet.nFieldID = nFieldID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SERVERWAR_LEADER_LIST(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(867, stream);
...
function API_C_EX_SERVERWAR_SELECT_LEADER(int nFieldID, int nLeaderWorldID, int nLeaderDBID)
{
	...
	packet.nLeaderWorldID = nLeaderWorldID;
	packet.nLeaderDBID = nLeaderDBID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SERVERWAR_SELECT_LEADER(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(868, stream);
...
function API_C_EX_SERVERWAR_SELECT_LEADER_INFO(int nFieldID)
{
	...
	packet.nFieldID = nFieldID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SERVERWAR_SELECT_LEADER_INFO(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(869, stream);
...
function API_C_EX_SERVERWAR_REWARD_ITEM_INFO(int nFieldID)
{
	...
	packet.nFieldID = nFieldID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SERVERWAR_REWARD_ITEM_INFO(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(871, stream);
...
function API_C_EX_SERVERWAR_REWARD_INFO(int nFieldID)
{
	...
	packet.nFieldID = nFieldID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SERVERWAR_REWARD_INFO(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(872, stream);
...
function API_C_EX_SERVERWAR_GET_REWARD(int nFieldID)
{
	...
	packet.nFieldID = nFieldID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SERVERWAR_GET_REWARD(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(873, stream);
...
function OnDialogOK()
{
	...
	WaitingList.GetRec(WaitingList.GetSelectedIndex(), rowData);
	API_C_EX_SERVERWAR_SELECT_LEADER(Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerFieldID, int(rowData.nReserved2), int(rowData.nReserved3));
	OnClickCancelDialog();
	return;
}
...
state cReward
{
	function BeginState()
	{
		...
		setRewardButtonState();
		setRewardItemInfo();
		waitingRemainTimeSec = Class'InterfaceClassic.NoticeHUD'.static.Inst().SkyTowerRemainTime;
		Me.KillTimer(123445);
...
state cWaiting
{
	function BeginState()
	{
		...
		Me.KillTimer(123445);
		Me.SetTimer(123445, 1000);
		if((Class'InterfaceClassic.NoticeHUD'.static.Inst().skyTowerCurrentState == 3))
		{
