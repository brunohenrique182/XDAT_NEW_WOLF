...
function OnShowFixedRearItemMessage(string param)
{
	...
	Class'InterfaceClassic.OnscreenEffectViewPortWnd'.static.Inst()._playEffectViewWithText(1.0000000, 1999.0000000, "LineageEffect2.ui_screen_message_flow", 0, 0, 0, -10, 0, 3000, 500, msgStr, L2Util(GetScript("L2Util")).White, GetItemInfoByClassID(ItemClassID), , true);
	return;
}
...
function HandleShopCraftLimitServerAnnounce()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SERVERLIMIT_ITEM_ANNOUNCE(packet))
	{
		return;
	}
	param = "";
...
function AddSystemSERVERLIMITMessage(string sUserName, int ItemClassID, INT64 getAmount, string amountString)
{
	...
	ItemName = Class'NWindow.UIDATA_ITEM'.static.GetItemName(Class'InterfaceClassic.UICommonAPI'.static.GetItemID(ItemClassID));
...
function HandleRaidDropItem()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_RAID_DROP_ITEM_ANNOUNCE(packet))
	{
		return;
	}
