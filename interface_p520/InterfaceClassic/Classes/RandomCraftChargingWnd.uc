...
function MoveItemTopToBottom(int Index, bool bAllItem)
{
	...
	if(m_topList.GetItem(Index, topInfo))
	{
		if(((!bAllItem && IsStackableItem(topInfo.ConsumeType)) && (topInfo.ItemNum > INT64(1))))
		{
			...
			Class'InterfaceClassic.DialogBox'.static.Inst().SetEditMessage(string(toAddNum));
...
function MoveItemBottomToTop(int Index, bool bAllItem)
{
	...
	if(m_bottomList.GetItem(Index, bottomInfo))
	{
		if(((!bAllItem && IsStackableItem(bottomInfo.ConsumeType)) && (bottomInfo.ItemNum > INT64(1))))
		{
			...
			Class'InterfaceClassic.DialogBox'.static.Inst().SetEditMessage(string(bottomInfo.ItemNum));
			Class'InterfaceClassic.DialogBox'.static.Inst()._AllSelect();
		}
		else
		{
...
function API_C_EX_CRAFT_EXTRACT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_CRAFT_EXTRACT(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(619, stream);
	return;
}
...
function ParsePacket_S_EX_CRAFT_EXTRACT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_CRAFT_EXTRACT(packet))
	{
		return;
	}
