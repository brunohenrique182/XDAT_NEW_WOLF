...
function HandleBuyBtn()
{
	if(!bAttendToday)
	{
		Class'InterfaceClassic.L2Util'.static.Inst().showGfxScreenMessage(GetSystemMessage(13848));
		return;
	}
...
function Rq_C_EX_VIP_ATTENDANCE_LIST()
{
	...
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(846, stream);
	return;
}

function Rt_S_EX_VIP_ATTENDANCE_LIST()
{
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_VIP_ATTENDANCE_LIST(packet_attendanceList))
	{
		return;
	}
...
function Rq_C_EX_VIP_ATTENDANCE_CHECK()
{
	...
	if(!IsLevelCondition())
	{
		Class'InterfaceClassic.L2Util'.static.Inst().showGfxScreenMessage(GetSystemMessage(6129));
		return;
	}
	packet.cDay = (packet_attendanceList.cAttendanceDay + 1);
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_VIP_ATTENDANCE_CHECK(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(847, stream);
	return;
}
...
function Rt_S_EX_VIP_ATTENDANCE_CHECK()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_VIP_ATTENDANCE_CHECK(packet))
	{
		return;
	}
...
function Rq_C_EX_VIP_ATTENDANCE_REWARD()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_VIP_ATTENDANCE_REWARD(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(848, stream);
	return;
}
...
function Rt_S_EX_VIP_ATTENDANCE_REWARD()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_VIP_ATTENDANCE_REWARD(packet))
	{
		return;
	}
	if((int(packet.bResult) == 1))
...
function InitSlots()
{
	...
	while((i < 28))
	{
		attendCheckSlots[i] = new Class'InterfaceClassic.AttendCheckSlot';
...
function InitTimer()
{
	tObject = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(1000, 0);
	tObject._DelegateOnTime = HandleOnTimer;
	tObject._Stop();
	tObjectReward = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(150, 0);
...
function InitAssets()
{
	...
	buyBtnDialogAsset = Class'InterfaceClassic.UIControlDialogAssets'.static.InitScript(buyBtnWnd);
