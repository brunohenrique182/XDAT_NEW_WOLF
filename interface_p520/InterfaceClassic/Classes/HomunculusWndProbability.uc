...
function Handle_S_EX_HOMUNCULUS_CREATE_PROB_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_HOMUNCULUS_CREATE_PROB_LIST(packet))
	{
		return;
	}
...
function Handle_S_EX_HOMUNCULUS_COUPON_PROB_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_HOMUNCULUS_COUPON_PROB_LIST(packet))
	{
		return;
	}
...
function API_C_EX_REQ_HOMUNCULUS_PROB_LIST(int nType, optional int nSlotItemClassId)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_REQ_HOMUNCULUS_PROB_LIST(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(773, stream);
	return;
}
