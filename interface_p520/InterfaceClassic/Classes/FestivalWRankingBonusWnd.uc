class FestivalWRankingBonusWnd extends UICommonAPI
	dependson(UIPacket);
...
var INT64 myAmount;
var array<UIPacket._WRFBonusInfo> _bonusInfos;
...
event OnShow()
{
	getInstanceL2Util().windowMoveToSide(GetWindowHandle("FestivalWRankingWnd"), Me);
...
event OnClickButton(string Name)
{
	...
	if((Left(Name, 7) == "btnInfo"))
	{
		...
		festivalWRankingWndScript.Rq_C_EX_WRANKING_FESTIVAL_BONUS(Points);
	}
...
function SetBonusList(array<UIPacket._WRFBonusInfo> bonusInfos)
{
	local int i;
	local ItemInfo iInfo;
...
function CheckCanReard()
{
	...
	{
		festivalWRankingWndScript.SetGiftBox(false);
		TakeAll_Btn.DisableWindow();
	}
	else
	{
		festivalWRankingWndScript.SetGiftBox(true);
