...
const LCOIN_CLASSID = 91663;
const MAXITEMNUM = 10000;
...
event OnLoad()
{
	...
	itemSell_NumberInputScr = Class'InterfaceClassic.UIControlNumberInput'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".RegiTabWnd.ItemRegi_Wnd.ItemSell_NumberInput")));
	bundlePrice_NumberInputScr = Class'InterfaceClassic.UIControlNumberInput'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".RegiTabWnd.ItemRegi_Wnd.BundlePrice_NumberInput")));
	...
	AddItemListenerSimple(91663, 0, 91663).DelegateOnUpdateItem = HandleMyItemChanged;
	tObject = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(1000);
...
function ItemDrop(ItemInfo Info)
{
	...
	else if(IsAdena(Info.Id))
	{
		...
		Class'InterfaceClassic.DialogBox'.static.Inst().DelegateOnOK = HandleDialogOKSource;
		Debug((("ItemDrop" @ string(sellItemInfo.ItemNum)) @ string(Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ADENA_MAX)));
		DialogSetInputlimit(Min64(sellItemInfo.ItemNum, Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ADENA_MAX));
		DialogSetParamInt64(Min64(sellItemInfo.ItemNum, Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ADENA_MAX));
	}
	else
	{
		Class'InterfaceClassic.DialogBox'.static.Inst().SetDefaultAction(EDefaultOK);
...
event OnDropItem(string strTarget, ItemInfo iInfo, int X, int Y)
{
	...
	else
	{
		...
		if(IsAdena(iInfo.Id))
		{
			if((iInfo.ItemNum < Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ADENA_BASIC_UNIT))
			{
			...
			{
				DialogShow(DialogModalType_Modalless, DialogType_NumberPadAdena, GetSystemString(14423));
			}
			else
			{
				DialogShow(DialogModalType_Modalless, DialogType_NumberPadAdena, GetSystemString(14193));
			}
			DialogSetInputlimit(Min64(iInfo.ItemNum, Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ADENA_MAX));
			DialogSetParamInt64(Min64(iInfo.ItemNum, Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ADENA_MAX));
		}
		else
		{
			DialogShow(DialogModalType_Modalless, DialogType_NumberPad, MakeFullSystemMsg(GetSystemMessage(72), iInfo.Name, ""));
			DialogSetInputlimit(Min64(INT64(10000), iInfo.ItemNum));
			DialogSetParamInt64(Min64(INT64(10000), iInfo.ItemNum));
		}
		Class'InterfaceClassic.DialogBox'.static.Inst().AnchorToOwner();
		Class'InterfaceClassic.DialogBox'.static.Inst().DelegateOnOK = HandleDialogOKDrop;
	}
	return;
}
...
function RQ_C_EX_WORLD_EXCHANGE_REGI_ITEM()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_WORLD_EXCHANGE_REGI_ITEM(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(785, stream);
	return;
}
...
function RT_S_EX_WORLD_EXCHANGE_REGI_ITEM()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_WORLD_EXCHANGE_REGI_ITEM(packet))
	{
		return;
	}
...
function RQ_C_EX_WORLD_EXCHANGE_AVERAGE_PRICE()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_WORLD_EXCHANGE_AVERAGE_PRICE(stream, packet))
	{
		return;
	}
	Debug(("RQ_C_EX_WORLD_EXCHANGE_AVERAGE_PRICE" @ string(packet.nItemID)));
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(812, stream);
...
function RT_S_EX_WORLD_EXCHANGE_AVERAGE_PRICE()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_WORLD_EXCHANGE_AVERAGE_PRICE(packet))
	{
		return;
	}
...
function SetItemInfo()
{
	...
	if(IsAdena(sellItemInfo.Id))
	{
		itemSell_NumberInputScr._SetMinCountCanBuy(Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ADENA_MIN);
		itemSell_NumberInputScr._SetPlusBasicUnit(Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ADENA_BASIC_UNIT);
	}
	else
	{
...
function INT64 GetSellItemInfoNum(int sererID)
{
	...
	if(Class'NWindow.UIDATA_INVENTORY'.static.FindItem(sellItemInfo.Id.ServerID, iInfo))
	{
		if(IsAdena(iInfo.Id))
		{
			return Min64(iInfo.ItemNum, Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ADENA_MAX);
		}
		else
		{
			return Min64(iInfo.ItemNum, INT64(10000));
		}
	}
	return INT64(0);
}
...
function int GetCommitionSell()
{
	...
	tmpWorldExchangeUIData = API_GetWorldExchangeData();
	return Min(tmpWorldExchangeUIData.MaxSellFee, int((float(bundlePrice_NumberInputScr.GetCount()) * (float(tmpWorldExchangeUIData.SellFee) / 100.0000000))));
}
...
function TInsertRecord()
{
	...
	if(IsAdena(sellItemInfo.Id))
	{
		...
		while((i < Cnt))
		{
			if(Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst()._MakeRowDataAdena(_itemDatas[i], rowData))
			{
...
function bool MakeRowData(UIPacket._WorldExchangeItemData _itemData, out RichListCtrlRowData outRowData)
{
	...
	strs = Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst()._GetDividedValue2Strs(iInfo.Price, iInfo.ItemNum, 3);
...
function CheckWorldExchangeRegiSubWnd()
{
	...
	{
		Class'InterfaceClassic.WorldExchangeRegiSubWnd'.static.Inst()._Show();
	}
	else
	{
		Class'InterfaceClassic.WorldExchangeRegiSubWnd'.static.Inst()._Hide();
	}
	return;
}
...
function SetMyInfos()
{
	...
	if((Class'NWindow.UIDATA_INVENTORY'.static.FindItemByClassID(91663, iInfos) > 0))
	{
...
function HandleOnitemCountEdited(INT64 changedNum)
{
	...
	Class'InterfaceClassic.WorldExchangeRegiSubWnd'.static.Inst()._ResetSellItem();
...
function HandleOnItemCountEditBtonClicked()
{
	...
	Class'InterfaceClassic.DialogBox'.static.Inst().AnchorToOwner();
	Class'InterfaceClassic.DialogBox'.static.Inst().DelegateOnOK = HandleDialogOkEdit;
	return;
}
...
function HandleOnPriceCountEditBtonClicked()
{
	...
	Class'InterfaceClassic.DialogBox'.static.Inst().AnchorToOwner();
	Class'InterfaceClassic.DialogBox'.static.Inst().DelegateOnOK = HandleDialogOKPrice;
	return;
}

function CheckGetRewardBtn()
{
	if(((itemSell_NumberInputScr.GetCount() == INT64(0)) || (GetCommitionSell() == 0)))
	{
		GetButtonHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".RegiTabWnd.ItemRegi_Wnd.GetReward_Btn")).DisableWindow();
	}
	else
...
function HandleClickOK_Btn()
{
	...
	if(IsAdena(sellItemInfo.Id))
	{
		perPrice = (perPrice * float(Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ADENA_BASIC_UNIT));
	}
	...
	if((perPrice < (nAveragePrice * 0.9000000)))
	{
		...
		Class'InterfaceClassic.DialogBox'.static.Inst().AnchorToOwner(-7, 144);
		Class'InterfaceClassic.DialogBox'.static.Inst().DelegateOnOK = HandleOK;
		Class'InterfaceClassic.DialogBox'.static.Inst().DelegateOnCancel = SetHideSellDialogWindow;
		TweenAdd();
	}
	else
	{
...
function TweenAdd()
{
	Class'InterfaceClassic.L2UITween'.static.Inst()._AddTweenTwinlkle(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".ItemRegiDialog_Wnd.Dialog_Wnd.UnitPriceCount_txt")), -1.0000000);
	Class'InterfaceClassic.L2UITween'.static.Inst()._AddTweenTwinlkle(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".ItemRegiDialog_Wnd.Dialog_Wnd.UnitPricePrimeNumber_txt")), -1.0000000);
	return;
}
...
function TweenCancel()
{
	...
	Class'InterfaceClassic.L2UITween'.static.Inst()._KillTwinkleWithWnd(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".ItemRegiDialog_Wnd.Dialog_Wnd.UnitPriceCount_txt")));
	Class'InterfaceClassic.L2UITween'.static.Inst()._KillTwinkleWithWnd(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".ItemRegiDialog_Wnd.Dialog_Wnd.UnitPricePrimeNumber_txt")));
...
function _SetUnity(INT64 bundlePrice, INT64 ItemNum)
{
	...
	if(IsAdena(sellItemInfo.Id))
	{
		ItemNum = (ItemNum / Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ADENA_BASIC_UNIT);
		if(Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst()._IsNewServer())
		{
	...
	strs = Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst()._GetDividedValue2Strs(bundlePrice, ItemNum, 3);
	Get_UnitPriceCount_Txt().SetText(strs[0]);
	Get_UnitPricePrimeNumber_txt().SetText(("." $ strs[1]));
	return;
}
