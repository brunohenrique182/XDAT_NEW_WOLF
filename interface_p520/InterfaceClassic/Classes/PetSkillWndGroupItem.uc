class PetSkillWndGroupItem extends UICommonAPI;

const SKILL_SLOT_COLUMN_NUM = 6;
const SKILL_SLOT_GAP_Y = 12;
const SKILL_SLOT_HEIGHT = 48;

var PetSkillGroupInfo__PetSkillWnd _info;
var array<PetSkillSlotInfo__PetSkillWnd> _slotInfos;
var WindowHandle Me;
...
delegate DelegateOnSkillInfoClicked(PetSkillSlotInfo__PetSkillWnd skillSlotInfo)
{
	return;
}
...
function SetGroupInfo(PetSkillGroupInfo__PetSkillWnd Info)
{
	local int i, windowHeight, SlotIndex, Row, prevGroup, nextGroup;
	local PetSkillSlotInfo__PetSkillWnd tmpSkillInfo;
	...
	while((i < _slotInfos.Length))
	{
		...
		if(((tmpSkillInfo.isAction == false) && (tmpSkillInfo.SkillInfo.SkillID == 0)))
		{
...
function byte MakeSkillStateBitflag(PetSkillSlotInfo__PetSkillWnd slotInfo)
{
	local byte bitflag;
...
function PetSkillSlotInfo__PetSkillWnd GetPetSkillSlotInfo(int ClassID)
{
...
event OnClickItem(string strID, int Index)
{
	...
	skillSlotInfo = GetPetSkillSlotInfo(Info.Id.ClassID);
	if((skillSlotInfo.isAction == true))
	{
		DoAction(skillSlotInfo.skillItemInfo.Id);
		Debug(("-- DoAction" @ string(skillSlotInfo.skillItemInfo.Id.ClassID)));
	}
	else if(((skillSlotInfo.learned == true) && (skillSlotInfo.isActiveSkill == true)))
	{
