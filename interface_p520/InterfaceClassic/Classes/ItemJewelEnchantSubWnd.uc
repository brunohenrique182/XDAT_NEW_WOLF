...
function _Refresh()
{
	...
	if(_GetCanEnchantItemCurrent(nCommissionAdena, itemInfoArray))
	{
		...
		{
			if((int(Class'InterfaceClassic.ItemJewelEnchantWnd'.static.Inst().currentEnchantType) == 1))
			{
...
{
	return Class'InterfaceClassic.ItemJewelEnchantWnd'.static.Inst().GetStateName();
}
...
function CheckNRemoveSameAutoInventory(out array<ItemInfo> MaterialItems)
{
	...
	Len = Class'InterfaceClassic.ItemJewelEnchantWnd'.static.Inst().InventoryItem.GetItemNum();
	j = 0;
	while((j < Len))
	{
		if(Class'InterfaceClassic.ItemJewelEnchantWnd'.static.Inst().InventoryItem.GetItem(j, iInfo))
		{
			i = 0;
			while((i < MaterialItems.Length))
			{
...
{
	return Class'InterfaceClassic.ItemJewelEnchantWnd'.static.Inst()._IsAutoMode();
}
