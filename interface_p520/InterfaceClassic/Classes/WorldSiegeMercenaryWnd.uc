...
event OnShow()
{
	...
	Class'InterfaceClassic.WorldSiegeInfoMCWWnd'.static.Inst().API_C_EX_WORLDCASTLEWAR_CASTLE_SIEGE_ATTACKER_LIST();
	m_hOwnerWnd.SetFocus();
	return;
}
...
event OnDBClickListCtrlRecord(string ListCtrlID)
{
	...
	Class'InterfaceClassic.WorldSiegeMercenaryDrawerWnd'.static.Inst().SetSiegeMercenary(int(rowData.nReserved1), ClanName, ConvertWorldIDToStr(PledgeMasterName));
	return;
}
