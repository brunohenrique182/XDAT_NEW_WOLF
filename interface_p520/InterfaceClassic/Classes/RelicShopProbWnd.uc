...
function ShowInfo(array<UIPacket._RelicsProb> relicProbs)
{
	...
	if((probLists.Length > 0))
	{
		...
		while((i < probLists.Length))
		{
			...
			tmpInfo = probLists[i];
			AddRichListCtrlString(rowData.cellDataList[0].drawitems, util.GetDollNameWithGrade(tmpInfo.relicName, currentGrade), util.GetRelicTextColor(currentGrade), false, 10);
...
function InsertTitleRecord(UIConstants.ERelicGrade currentGrade)
{
	...
	AddRichListCtrlString(rowData.cellDataList[0].drawitems, GetSystemString(getInstanceL2Util().GetDollGradeStringId(currentGrade)), getInstanceL2Util().BWhite, false, 8, , "hs12");
	RichListCtrl.InsertRecord(rowData);
	return;
}
