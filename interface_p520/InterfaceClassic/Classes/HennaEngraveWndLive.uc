...
function Initialize()
{
	...
	timeObjectHiddenEnchantingDelay = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(400, 1);
	timeObjectHiddenEnchantingDelay._DelegateOnTime = onTimeObjectHiddenEnchantingDelay;
	timeObjectEnchantingDelay = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(400, 1);
	timeObjectEnchantingDelay._DelegateOnTime = onTimeObjectEnchantingDelay;
	timeObjectAutoEnchantDelay = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(300, 1);
...
function Load()
{
	...
	InitNeedItemList();
	HennaGaugeWndScript = Class'InterfaceClassic.HennaGaugeWnd'.static._InitScript(GetMeWindow("GaugeWnd"));
	HennaGaugeWndScript.initPiece();
	HennaGaugeWndPopupScript = Class'InterfaceClassic.HennaGaugeWnd'.static._InitScript(GetWindowHandle("DisableWnd.EnchantWnd.EnchantGaugeWnd"));
...
{
	NeedItemListWndScript = new Class'InterfaceClassic.UIControlNeedItemList';
	NeedItemListWndScript.SetRichListControler(GetMeRichListCtrl("NeedItemListWnd.Cost_RichListCtrl"));
	EnchantNeedItemRichListCtrlScript = new Class'InterfaceClassic.UIControlNeedItemList';
	...
	ResetNeedItemRichListCtrlScript = new Class'InterfaceClassic.UIControlNeedItemList';
	ResetNeedItemRichListCtrlScript.SetRichListControler(GetMeRichListCtrl("DisableWnd.ResetWnd.ResetNeedItemRichListCtrl"));
	ResetPayBackItemRichListCtrlScript = new Class'InterfaceClassic.UIControlNeedItemList';
	...
	HiddenEnchantNeedItemListCtrlScript = new Class'InterfaceClassic.UIControlNeedItemList';
...
{
	sideGroupButtons = new Class'InterfaceClassic.UIControlGroupButtons';
	...
	topGroupButtons = new Class'InterfaceClassic.UIControlGroupButtons';
...
function ParsePacket_S_EX_DYEEFFECT_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_DYEEFFECT_LIST(PACKET_DYEEFFECT_LIST))
	{
		return;
	}
...
function ParsePacket_S_EX_DYEEFFECT_ENCHANT_PROB_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_DYEEFFECT_ENCHANT_PROB_INFO(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_DYEEFFECT_ENCHANT_NORMALSKILL()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_DYEEFFECT_ENCHANT_NORMALSKILL(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_DYEEFFECT_ACQUIRE_HIDDENSKILL()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_DYEEFFECT_ACQUIRE_HIDDENSKILL(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_DYEEFFECT_ENCHANT_RESET()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_DYEEFFECT_ENCHANT_RESET(packet))
	{
		return;
	}
...
function API_C_EX_DYEEFFECT_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_DYEEFFECT_LIST(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(907, stream);
...
function API_C_EX_DYEEFFECT_ENCHANT_PROB_INFO(int nCategory, int nSlotID)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_DYEEFFECT_ENCHANT_PROB_INFO(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(908, stream);
...
function API_C_EX_DYEEFFECT_ENCHANT_NORMALSKILL(int nCategory, int nSlotID)
{
	...
	packet.nCategory = nCategory;
	packet.nSlotID = nSlotID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_DYEEFFECT_ENCHANT_NORMALSKILL(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(909, stream);
...
function API_C_EX_DYEEFFECT_ENCHANT_RESET(int nCategory, int nSlotID)
{
	...
	packet.nCategory = nCategory;
	packet.nSlotID = nSlotID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_DYEEFFECT_ENCHANT_RESET(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(911, stream);
...
function API_C_EX_DYEEFFECT_ACQUIRE_HIDDENSKILL(int nCategory, int nSlotID)
{
	...
	packet.nCategory = nCategory;
	packet.nSlotID = nSlotID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_DYEEFFECT_ACQUIRE_HIDDENSKILL(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(910, stream);
...
function SetUserInfo()
{
	...
	Class'InterfaceClassic.SideBar'.static.Inst().GetWindowByIndex(8).SetTooltipCustomType(getSideBarCustomTooltip(Info.nDyeChargeAmount, maxDyeChargeAmount));
