...
event OnDBClickItem(string ControlName, int Index)
{
	...
	Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst().OnDropItem("", iInfo, 0, 0);
	return;
}
...
function _ResetSellItem()
{
	...
	Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst()._GetSellItemInfo(sellItemInfo);
	if(!Class'NWindow.UIDATA_INVENTORY'.static.HasItem(sellItemInfo.Id.ServerID))
	{
		Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst().DelItemInfo();
		refresh();
		return;
	}
