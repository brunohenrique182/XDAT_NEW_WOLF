...
{
	numberInputStepper = Class'InterfaceClassic.UIControlNumberInputSteper'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".EnchantSystem_Wnd.EnchantGoalSet_wndAsset")));
...
event OnLoad()
{
	...
	SetGrouIDWnd();
	GotoState('stateNone');
	timerObj = Class'InterfaceClassic.L2UITimer'.static.Inst()._MakeTimerObject(2000);
...
function SwapItemEnchantWnd()
{
	...
	{
		Class'InterfaceClassic.ItemEnchantWnd'.static.Inst().API_RequestExAddEnchantScrollItem(iInfo);
	}
	return;
}
...
function HandleRequestedEquipment()
{
	...
	Class'InterfaceClassic.ItemMultiEnchantSubWnd'.static.Inst().refresh();
	return;
}
...
function SetSlotEmpty(ItemInfo iInfo)
{
	...
	{
		Class'InterfaceClassic.L2Util'.static.Inst().showGfxScreenMessage(GetSystemMessage(13648));
		return;
	}
	...
	if((Index == -1))
	{
		Class'InterfaceClassic.L2Util'.static.Inst().showGfxScreenMessage(GetSystemMessage(13647));
		return;
	}
	...
	switch(slotstate)
	{
		...
		case active:
			...
			if(!bIsActive)
			{
				...
				ProbTxt.SetTextColor(Class'InterfaceClassic.L2Util'.static.Inst().Gray);
			...
			else
			{
				...
				ProbTxt.SetTextColor(Class'InterfaceClassic.L2Util'.static.Inst().Yellow);
				ProbTxt.ShowWindow();
...
function SetNeedScrollNum()
{
	...
	{
		txtHandle.SetTextColor(Class'InterfaceClassic.L2Util'.static.Inst().Red);
	}
	else
	{
		txtHandle.SetTextColor(Class'InterfaceClassic.L2Util'.static.Inst().White);
	}
...
function RQ_C_EX_REQ_START_MULTI_ENCHANT_SCROLL(ItemInfo iInfo)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_REQ_START_MULTI_ENCHANT_SCROLL(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(765, stream);
	return;
}
...
function RQ_C_EX_REQ_CHANGE_MULTI_ENCHANT_SCROLL(ItemInfo iInfo)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_REQ_CHANGE_MULTI_ENCHANT_SCROLL(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(768, stream);
	return;
}
...
function RT_S_EX_RES_SELECT_MULTI_ENCHANT_SCROLL()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_RES_SELECT_MULTI_ENCHANT_SCROLL(packet))
	{
		return;
	}
...
function RQ_C_EX_REQ_SET_MULTI_ENCHANT_ITEM_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_REQ_SET_MULTI_ENCHANT_ITEM_LIST(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(769, stream);
	return;
}
...
function RT_S_EX_RES_SET_MULTI_ENCHANT_ITEM_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_RES_SET_MULTI_ENCHANT_ITEM_LIST(packet))
	{
		return;
	}
	resetBtn.EnableWindow();
	switch(packet.nResult)
	{
		case 0:
			...
			break;
		case 1:
			Class'InterfaceClassic.L2Util'.static.Inst().showGfxScreenMessage(GetSystemMessage(184));
			break;
		case 2:
			Class'InterfaceClassic.L2Util'.static.Inst().showGfxScreenMessage(GetSystemMessage(13649));
			break;
		case 3:
			Class'InterfaceClassic.L2Util'.static.Inst().showGfxScreenMessage(GetSystemMessage(3675));
			break;
		case 4:
			Class'InterfaceClassic.L2Util'.static.Inst().showGfxScreenMessage(GetSystemMessage(13648));
			break;
...
function RQ_C_EX_REQ_MULTI_ENCHANT_ITEM_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_REQ_MULTI_ENCHANT_ITEM_LIST(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(770, stream);
	return;
}
...
function RT_S_EX_RES_MULTI_ENCHANT_ITEM_LIST()
{
	...
	EnchantBtn.EnableWindow();
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_RES_MULTI_ENCHANT_ITEM_LIST(packet))
	{
		return;
	}
	...
	switch(packet.bResult)
	{
		...
		case 1:
			...
			Class'InterfaceClassic.L2UITimer'.static.Inst()._AddTimerOnce()._DelegateOnEnd = RemoveFailedItems;
...
function SetFailItemList(array<UIPacket._EnchantFailChallengePointInfo> vFailChallengePointInfo, array<UIPacket._EnchantFailRewardItem> failRewardItems)
{
	...
	{
		Class'NWindow.UIDATA_ITEM'.static.GetItemInfo(GetItemID(Class'InterfaceClassic.ItemEnchantGroupIDWnd'.static._GetClassIDPoint(vFailChallengePointInfo[i].nGroupID)), iInfo);
...
function RQ_C_EX_REQ_VIEW_MULTI_ENCHANT_RESULT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_REQ_VIEW_MULTI_ENCHANT_RESULT(stream, packet))
	{
		return;
	}
	bUseLateAnnounce = false;
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(766, stream);
	NoticeWnd(GetScript("NoticeWnd"))._CreateCollectionButtonBlind();
	return;
}
...
function RQ_C_EX_REQ_FINISH_MULTI_ENCHANT_SCROLL()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_REQ_FINISH_MULTI_ENCHANT_SCROLL(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(767, stream);
	return;
}
...
state stateReadyScroll
{
	function BeginState()
	{
		...
		SetCurrentEnchantTarget(1);
		SetNeedScrollNum();
		Class'InterfaceClassic.ItemMultiEnchantSubWnd'.static.Inst().Show();
		return;
	}
...
state stateEnchant
{
	function BeginState()
	{
		...
		Class'InterfaceClassic.ItemMultiEnchantSubWnd'.static.Inst().Hide();
		currentEnchantTarget = 1;
		ResetProgressStates();
		StartProgress();
		return;
