...
{
	TopGroupButtonAsset = Class'InterfaceClassic.UIControlGroupButtonAssets'.static._InitScript(GetMeWindow("TopGroupButtonAsset"));
	...
	SubGroupButtonAsset = Class'InterfaceClassic.UIControlGroupButtonAssets'.static._InitScript(GetMeWindow("ItemRecovery_Wnd.SubGroupButtonAsset"));
...
function ParsePacket_S_EX_ITEM_RESTORE_OPEN()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_ITEM_RESTORE_OPEN(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_ITEM_RESTORE_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_ITEM_RESTORE_LIST(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_ITEM_RESTORE()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_ITEM_RESTORE(packet))
	{
		return;
	}
...
function API_C_EX_ITEM_RESTORE_LIST(int cCategory)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_ITEM_RESTORE_LIST(stream, packet))
	{
		return;
	}
	RecoveryRichList.DeleteAllItem();
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(713, stream);
...
function API_C_EX_ITEM_RESTORE(int nBrokenItemClassID, int cEnchant)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_ITEM_RESTORE(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(714, stream);
