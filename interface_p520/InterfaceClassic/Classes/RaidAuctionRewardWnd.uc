...
{
	uicontrolDialogAssetScr = Class'InterfaceClassic.UIControlDialogAssets'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".DisableWnd.UIControlDialogAsset")));
...
function HandleCLickRefreshBtn()
{
	...
	Class'InterfaceClassic.L2UITimer'.static.Inst()._AddTimerOnce(5000)._DelegateOnEnd = EnableRefreshBtn;
...
{
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(918, _emptyByteArray);
	return;
}
...
function RQ_C_EX_RAID_AUCTION_POST_RECEIVE(bool bAccept)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_RAID_AUCTION_POST_RECEIVE(stream, packet))
	{
		return;
	}
	Debug((("RQ_C_EX_RAID_AUCTION_POST_RECEIVE" @ string(packet.nID)) @ string(packet.bAccept)));
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(919, stream);
	return;
}

function RQ_C_EX_RAID_AUCTION_POST_RECEIVE_ALL()
{
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(920, _emptyByteArray);
	return;
}
...
function Rt_S_EX_RAID_AUCTION_POST_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_RAID_AUCTION_POST_LIST(packet))
	{
		return;
	}
	RewardRichList.DeleteAllItem();
