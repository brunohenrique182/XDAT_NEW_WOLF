...
function CloseAndResetInfo()
{
	...
	_remainTimerCount = 0;
	Class'InterfaceClassic.PrisonWnd'.static.Inst().ClosePrisonWnd();
	return;
}
...
function Rq_C_EX_PRISON_USER_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_PRISON_USER_INFO(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(814, stream);
	return;
}
...
function Rs_S_EX_PRISON_USER_ENTER()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_PRISON_USER_ENTER(packet))
	{
		return;
	}
	_isEnterPacket = true;
...
function Rs_S_EX_PRISON_USER_EXIT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_PRISON_USER_EXIT(packet))
	{
		return;
	}
	CloseAndResetInfo();
...
function Rs_S_EX_PRISON_USER_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_PRISON_USER_INFO(packet))
	{
		return;
	}
	...
	if((_prisonInfo.inPrison == true))
	{
		...
		{
			_isEnterPacket = false;
			Class'InterfaceClassic.PrisonWnd'.static.Inst().OpenPrisonWnd();
		}
	}
	return;
}
...
event OnTimer(int TimerID)
{
	if((TimerID == 1))
	{
		...
		UpdateInfoControls();
		Class'InterfaceClassic.PrisonWnd'.static.Inst().UpdatePrisonPanelControls();
	}
	return;
}
...
{
	Class'InterfaceClassic.PrisonWnd'.static.Inst().ToggleOpenPrisonWnd();
	return;
}
