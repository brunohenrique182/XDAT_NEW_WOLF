...
function ParsePacket_OLYMPIAD_HERO_AND_LEGEND_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_OLYMPIAD_HERO_AND_LEGEND_INFO(packet))
	{
		return;
	}
...
function ParsePacket_OLYMPIAD_RANKING_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_OLYMPIAD_RANKING_INFO(packet))
	{
		return;
	}
	GetPlayerInfo(myInfo);
...
function ParsePacket_OLYMPIAD_MY_RANKING_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_OLYMPIAD_MY_RANKING_INFO(packet))
	{
		return;
	}
	GetPlayerInfo(myInfo);
...
function API_C_EX_OLYMPIAD_RANKING_INFO(OlympiadRankingType cRankingType, UIEventManager.RankingScope cRankingScope, bool bCurrentSeason, int nClassID, int nWorldID)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_OLYMPIAD_RANKING_INFO(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(614, stream);
...
function API_C_EX_OLYMPIAD_MY_RANKING_INFO()
{
	...
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(613, stream);
...
function API_C_EX_OLYMPIAD_HERO_AND_LEGEND_INFO()
{
	...
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(615, stream);
