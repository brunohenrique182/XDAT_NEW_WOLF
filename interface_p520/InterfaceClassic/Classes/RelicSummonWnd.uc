...
function OnHide()
{
	...
	Class'InterfaceClassic.RelicWnd'.static.Inst().CheckAndShowVisible();
	return;
}
...
function Initialize()
{
	...
	DollList_btn = GetMeButton("DollList_btn");
...
function setPlacedCardBack(int NumCards)
{
	...
	while((i <= 11))
	{
		GetMeEffectViewportWnd((("CardSlot_" $ fillZeroString(2, string(i))) $ ".CardOpen02_EffectViewport")).SpawnEffect("");
...
function setCardFront(int Index)
{
	...
	if(GetMeTexture((("CardSlot_" $ fillZeroString(2, string(Index))) $ ".CardBack_tex")).IsShowWindow())
	{
		...
		GetMeTexture((("CardSlot_" $ fillZeroString(2, string(Index))) $ ".Card_tex")).SetTexture(("L2UI_NewTex.RelicWnd.DollCard_" $ getGradeRelicsString(sortedCardArray[(Index - 1)].Grade)));
		dollName = getInstanceL2Util().GetDollNameWithGrade(GetItemNameAllByClassID(sortedCardArray[(Index - 1)].ClassID), ERelicGrade(sortedCardArray[(Index - 1)].Grade));
		GetMeButton((("CardSlot_" $ fillZeroString(2, string(Index))) $ ".CardOpenTooltip_btn")).ShowWindow();
		GetMeButton((("CardSlot_" $ fillZeroString(2, string(Index))) $ ".CardOpenTooltip_btn")).SetTooltipCustomType(MakeTooltipSimpleColorText(dollName, getInstanceL2Util().GetRelicTextColor(ERelicGrade(sortedCardArray[(Index - 1)].Grade))));
		...
		Debug(("sortedCardArray.Length" @ string(sortedCardArray.Length)));
		if(isHighGradeRelics(sortedCardArray[(Index - 1)].Grade))
		{
			AnimTexturePlay(GetMeAnimTexture((("CardSlot_" $ fillZeroString(2, string(Index))) $ ".CardEdgeEffect_ani")), true, 999999999);
			if((sortedCardArray.Length == 1))
			{
				if(((sortedCardArray[(Index - 1)].Grade == 6) || (sortedCardArray[(Index - 1)].Grade == 7)))
				{
					GetMeEffectViewportWnd((("CardSlot_" $ fillZeroString(2, string(Index))) $ ".CardOpen02_EffectViewport")).SetScale(4.0000000);
					GetMeEffectViewportWnd((("CardSlot_" $ fillZeroString(2, string(Index))) $ ".CardOpen02_EffectViewport")).SetCameraDistance(650.0000000);
					GetMeEffectViewportWnd((("CardSlot_" $ fillZeroString(2, string(Index))) $ ".CardOpen02_EffectViewport")).SpawnEffect("LineageEffect2.ui_adenlab_normal_success");
				}
				else
				{
					GetMeEffectViewportWnd((("CardSlot_" $ fillZeroString(2, string(Index))) $ ".CardOpen02_EffectViewport")).SetScale(4.0000000);
			...
			else if(((sortedCardArray[(Index - 1)].Grade == 6) || (sortedCardArray[(Index - 1)].Grade == 7)))
			{
				GetMeEffectViewportWnd((("CardSlot_" $ fillZeroString(2, string(Index))) $ ".CardOpen02_EffectViewport")).SetScale(2.0000000);
				GetMeEffectViewportWnd((("CardSlot_" $ fillZeroString(2, string(Index))) $ ".CardOpen02_EffectViewport")).SetCameraDistance(650.0000000);
				GetMeEffectViewportWnd((("CardSlot_" $ fillZeroString(2, string(Index))) $ ".CardOpen02_EffectViewport")).SpawnEffect("LineageEffect2.ui_adenlab_normal_success");
			}
			else
...
function addRichListRowDataTitle(int nGradeType)
{
	...
	rowData.sOverlayTex = ("L2UI_NewTex.RelicWnd.DollListHeaderBg_" $ getGradeRelicsString(nGradeType));
...
function addRichListRowDataHistory(int ClassID, INT64 nAmount, int nGrade)
{
	...
	dollName = getInstanceL2Util().GetDollNameWithGrade(GetItemNameAllByClassID(ClassID), ERelicGrade(nGrade));
	AddRichListCtrlItem(rowData.cellDataList[0].drawitems, Info, 32, 32, 4, 4);
	AddRichListCtrlString(rowData.cellDataList[0].drawitems, dollName, getInstanceL2Util().GetRelicTextColor(ERelicGrade(nGrade)), false, 7, 11);
...
function ParsePacket_S_EX_RELICS_SUMMON_RESULT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_RELICS_SUMMON_RESULT(packet))
	{
		return;
	}
	...
	else if((packet.cResult == 1))
	{
		...
		Class'InterfaceClassic.L2UITimer'.static.Inst()._AddTimerOnce(1)._DelegateOnEnd = cosumeItemUpdateAndCardSetting;
		sortedCardArray.Length = 0;
		i = 0;
...
function sumHistoryAutoOpenCardArray(cardStruct A)
{
	...
	while((i < historyAutoOpenCardArray.Length))
	{
		if(((historyAutoOpenCardArray[i].ClassID == A.ClassID) && (historyAutoOpenCardArray[i].Grade == A.Grade)))
		{
...
function ParsePacket_S_EX_RELICS_COMBINATION()
{
	...
	if((packet.cResult > 0))
	{
		...
		if(Class'InterfaceClassic.RelicWnd'.static.Inst().Me.IsShowWindow())
		{
			getInstanceL2Util().syncWindowLoc("RelicWnd", getCurrentWindowName(string(self)));
		}
	}
	...
	Class'InterfaceClassic.RelicWnd'.static.Inst().CheckAndHideVisible();
	return;
}
			...
			if((int(param) == currentGameCardNum))
			{
				...
				if(bIsAutoCardOpen)
				{
					...
					if(bAutoSummonMaxNumberType)
					{
						...
						{
							Class'InterfaceClassic.L2UITimer'.static.Inst()._AddTimerOnce(1000)._DelegateOnEnd = API_C_EX_RELICS_ID_SUMMON;
							hideAllButton();
					...
					{
						Class'InterfaceClassic.L2UITimer'.static.Inst()._AddTimerOnce(1000)._DelegateOnEnd = API_C_EX_RELICS_SUMMON;
						hideAllButton();
						AutoSummon_wnd.ShowWindow();
						AutoSummonCancle_btn.ShowWindow();
						AutoSummonHistory_btn.DisableWindow();
...
{
	CosumeItemListCtrl = new Class'InterfaceClassic.UIControlNeedItemList';
	...
	GetItemListCtrl = new Class'InterfaceClassic.UIControlNeedItemList';
...
function string getGradeRelicsSystemString(int Grade)
{
	...
	{
		case 1:
			return GetSystemString(14688);
		case 2:
			return GetSystemString(14689);
		case 3:
			return GetSystemString(14690);
		case 4:
			return GetSystemString(14691);
		case 5:
			return GetSystemString(14692);
		case 6:
			return GetSystemString(14693);
		case 7:
			return GetSystemString(14694);
		default:
			return "";
	}
}
...
function API_C_EX_RELICS_SUMMON_CLOSE_UI()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_RELICS_SUMMON_CLOSE_UI(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(857, stream);
...
function API_C_EX_RELICS_SUMMON()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_RELICS_SUMMON(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(859, stream);
...
function API_C_EX_RELICS_ID_SUMMON()
{
	...
	nCurrentSummonCount--;
	setAutoSummonNum();
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_RELICS_ID_SUMMON(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(902, stream);
...
function API_C_EX_RELICS_COMBINATION_COMPLETE()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_RELICS_COMBINATION_COMPLETE(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(874, stream);
