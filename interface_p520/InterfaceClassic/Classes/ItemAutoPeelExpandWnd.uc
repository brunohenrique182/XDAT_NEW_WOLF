...
function InitControls()
{
	...
	numberInput = Class'InterfaceClassic.UIControlNumberInput'.static.InitScript(GetWindowHandle((ownerFullPath $ ".NumberInputExpanded")));
...
function UpdateTargetItemControls()
{
	...
	Info = Class'InterfaceClassic.ItemAutoPeelWnd'.static.Inst().GetItemAutoPeelInfo();
...
function UpdateResultItemRichListControl(RichListCtrlHandle targetControl, array<UIPacket._AutoPeelResultItem> infos, bool isRare)
{
	...
	while((i < infos.Length))
	{
		...
		if((i < recordCnt))
		{
			...
			if(((oldRowData.nReserved1 == INT64(resultInfo.nItemClassID)) && (oldRowData.nReserved3 == INT64(resultInfo.Enchanted))))
			{
				...
				else
				{
					if(isRare)
					{
						oldRowData.sOverlayTex = Class'InterfaceClassic.ItemAutoPeelWnd'.static.Inst().GetGradeBGTextureName(resultInfo.gradeColor);
		...
		if(isRare)
		{
			newRowData.sOverlayTex = Class'InterfaceClassic.ItemAutoPeelWnd'.static.Inst().GetGradeBGTextureName(resultInfo.gradeColor);
		...
		Class'InterfaceClassic.L2Util'.static.GetEllipsisString(StrEllipsised, (300 - 4));
...
{
	return Class'InterfaceClassic.ItemAutoPeelWnd'.static.Inst().GetTargetItemNum();
}
...
event OnItemAutoPeelBtnClicked()
{
	...
	if((Info.targetItemSId != 0))
	{
		if((Info.isPeeling == true))
		{
			if((Info.isPause == true))
			{
				Class'InterfaceClassic.ItemAutoPeelWnd'.static.Inst().StartItemAutoPeel();
			}
			else
			{
				Class'InterfaceClassic.ItemAutoPeelWnd'.static.Inst().PauseItemAutoPeel();
			}
		}
		else
		{
...
{
	Class'InterfaceClassic.ItemAutoPeelWnd'.static.Inst().SetTargetTotalPeelCnt(ItemCount);
	return;
}
...
event OnResetBtnClicked()
{
	...
	StartBtnDelayTimer();
	Class'InterfaceClassic.ItemAutoPeelWnd'.static.Inst().ResetTargetItem();
	return;
}

event OnSetCountBtnClicked()
{
	Class'InterfaceClassic.ItemAutoPeelWnd'.static.Inst().ShowItemCountDialog();
	return;
}
...
{
	Class'InterfaceClassic.ItemAutoPeelWnd'.static.Inst().SetExpand(false);
	return;
}
