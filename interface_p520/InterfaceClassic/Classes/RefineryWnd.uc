...
function Rq_C_EX_TRY_TO_MAKE_VARIATION(int targetItemServerId, int intensiveItemServerId, bool isEventOn)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_TRY_TO_MAKE_VARIATION(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(271, stream);
	return;
}
...
function Decode_S_EX_SHOW_VARIATION_MAKE_WINDOW(string a_Param)
{
	...
	{
		if(Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SHOW_VARIATION_MAKE_WINDOW(packet))
		{
...
function Decode_S_EX_PUT_INTENSIVE_RESULT_FOR_VARIATION_MAKE(string a_Param)
{
	...
	Class'InterfaceClassic.UIPacket'.static.DecodeInt(targetItemServerId);
	Class'InterfaceClassic.UIPacket'.static.DecodeInt(TargetItemClassID);
	Class'InterfaceClassic.UIPacket'.static.DecodeInt(insertResult);
...
function Decode_S_EX_VARIATION_RESULT(string a_Param)
{
	...
	Class'InterfaceClassic.UIPacket'.static.DecodeInt(Option1);
	Class'InterfaceClassic.UIPacket'.static.DecodeInt(Option2);
	Class'InterfaceClassic.UIPacket'.static.DecodeInt(Option3);
	Class'InterfaceClassic.UIPacket'.static.DecodeInt64(GemStoneCount);
	Class'InterfaceClassic.UIPacket'.static.DecodeInt64(NecessaryGemStoneCount);
	Class'InterfaceClassic.UIPacket'.static.DecodeInt(RefineResult);
...
function SetInstructionText()
{
	...
	if((showSpecialOptionNotice == true))
	{
		Msg = ((Msg $ "\\n") $ GetSystemString(14843));
	}
...
{
	Debug(("OnReceivedCloseUI" @ string(CurrentState)));
	if((int(CurrentState) != 3))
	{
		SetHideWindow();
	}
