...
function OnITEMButton_ResetClick()
{
	...
	ResetMagicLampGetInfo();
	UpdateInfoControls();
	_resetGameConnectTimeSec = Class'InterfaceClassic.UIData'.static.Inst().gameConnectTimeSec();
	updateGameSessionText();
	return;
}
...
function updateGameSessionText()
{
	...
	gameConnectTimeSec = Class'InterfaceClassic.UIData'.static.Inst().gameConnectTimeSec();
...
function Nt_S_EX_MAGICLAMP_RESULT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_MAGICLAMP_RESULT(packet))
	{
		return;
	}
