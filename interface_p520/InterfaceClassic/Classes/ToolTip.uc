...
function ReturnTooltip_WorldExchangeHistory(string param, UIEventManager.ETooltipSourceType eSourceType)
{
	...
	if((int(eSourceType) == 3))
	{
		...
		{
			src = Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst().historyScr;
		...
		if((TooltipType != "InventoryPrice1HideEnchantStackable"))
		{
			if((TooltipType == "InventoryNeedItem"))
			{
				if(IsStackableItem(item.ConsumeType))
				{
					...
					m_Info.nOffSetX = 40;
					m_Info.nOffSetY = -16;
					m_Info.t_color = Class'InterfaceClassic.L2Util'.static.Inst().White;
					EndItem();
					StartItem();
					m_Info.eType = DIT_TEXT;
					...
					{
						m_Info.t_color = Class'InterfaceClassic.L2Util'.static.Inst().Red;
					}
					EndItem();
				}
			}
...
function ReturnTooltip_NTT_CLANWARList(string param, UIEventManager.ETooltipSourceType eSourceType)
{
	...
	if((int(eSourceType) == 2))
	{
		...
		addToolTipDrawList(m_Tooltip, addDrawItemText(MakeFullSystemMsg(GetSystemMessage(13869), Class'InterfaceClassic.UIData'.static.Inst().getMakeTimeString(UITimeStruct)), getInstanceL2Util().White));
	}
	ReturnTooltipInfo(m_Tooltip);
	return;
}
...
function ReturnTooltip_NTT_QUESTLIST(string param, UIEventManager.ETooltipSourceType eSourceType)
{
	...
	if((int(eSourceType) == 2))
	{
		...
		addToolTipDrawList(m_Tooltip, addDrawItemTextureCustom(Class'InterfaceClassic.QuestWnd'.static.Inst()._GetIconTextureByState(int(Record.nReserved3), isComplete), false, false, 0, 0, 24, 24, 24, 24));
...
function AddActiveRelicInfo(ItemInfo item)
{
	...
	activeRelicId = Class'InterfaceClassic.RelicWnd'.static.Inst().GetActiveRelicId();
	if((activeRelicId <= 0))
	{
		return;
	}
	relicSkillInfo = Class'InterfaceClassic.RelicWnd'.static.Inst().GetActiveRelicSkill();
...
function AddAdenLabCardEffectTooltip(ItemInfo iInfo)
{
	...
	if((Class'InterfaceClassic.UIData'.static.Inst()._GetCurrentTranscendEnchant() == 0))
	{
		return;
	}
	...
	bossNameString = Class'InterfaceClassic.UIData'.static.Inst()._GetAdenLabBossName(BossID);
...
function int GetEquipAddOptionData(ItemInfo iInfo, out EquipAddOptionData oEData)
{
	...
	API_GetTranscendStageData(Class'InterfaceClassic.UIData'.static.Inst()._GetCurrentStageIndex(), cardSelectTranscendStageData);
