...
event OnDBClickItem(string ControlName, int Index)
{
	...
	switch(Class'InterfaceClassic.ItemMultiEnchantWnd'.static.Inst().GetStateName())
	{
		case 'stateReadyScroll':
			Class'InterfaceClassic.ItemMultiEnchantWnd'.static.Inst().RQ_C_EX_REQ_START_MULTI_ENCHANT_SCROLL(iInfo);
			break;
		case 'stateReadyEquipment':
			Class'InterfaceClassic.ItemMultiEnchantWnd'.static.Inst().SetSlotEmpty(iInfo);
			break;
		default:
			break;
	}
...
function SetEnchantableItems()
{
	...
	while((i < iInfos.Length))
	{
		if((Class'InterfaceClassic.ItemMultiEnchantWnd'.static.Inst().FindIndexEquipmentWithServerID(iInfos[i].Id.ServerID) > -1))
		{
			i++;
			continue;
		}
...
function ItemInfo GetIteminfoScroll()
{
	...
	Class'InterfaceClassic.ItemMultiEnchantWnd'.static.Inst().scrollItemWindow.GetItem(0, scrollItem);
	return scrollItem;
}
