...
function API_C_EX_SHARED_POSITION_TELEPORT_UI(int nID)
{
	...
	packet.nID = nID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SHARED_POSITION_TELEPORT_UI(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(628, stream);
	return;
}
...
function API_C_EX_SHARED_POSITION_TELEPORT(int nID)
{
	...
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(629, stream);
	return;
}
...
function ParsePacket_S_EX_SHARED_POSITION_TELEPORT_UI()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SHARED_POSITION_TELEPORT_UI(packet))
	{
		return;
	}
