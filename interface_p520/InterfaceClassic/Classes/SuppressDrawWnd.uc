...
function addRichListReward(int nClassID, int nAmount, int ColorIndex, int probInt)
{
	...
	Class'InterfaceClassic.L2Util'.static.GetEllipsisString(ItemName, 400);
...
function API_C_EX_SUBJUGATION_GACHA_UI(int nID)
{
	...
	packet.nID = nID;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SUBJUGATION_GACHA_UI(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(706, stream);
...
function ParsePacket_S_EX_SUBJUGATION_GACHA_UI()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SUBJUGATION_GACHA_UI(packet))
	{
		return;
	}
...
function API_C_EX_SUBJUGATION_GACHA(int nID, int nCount)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_SUBJUGATION_GACHA(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(707, stream);
...
function ParsePacket_S_EX_SUBJUGATION_GACHA()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_SUBJUGATION_GACHA(packet))
	{
		return;
	}
...
function addRichListResult(int nClassID, int nAmount, int ColorIndex)
{
	...
	ItemName = GetItemNameAll(Info);
	Class'InterfaceClassic.L2Util'.static.GetEllipsisString(ItemName, 200);
