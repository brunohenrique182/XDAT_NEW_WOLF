...
function API_C_EX_LETTER_COLLECTOR_TAKE_REWARD(int setNo)
{
	...
	packet.nSetNo = setNo;
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_LETTER_COLLECTOR_TAKE_REWARD(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(610, stream);
	return;
}
