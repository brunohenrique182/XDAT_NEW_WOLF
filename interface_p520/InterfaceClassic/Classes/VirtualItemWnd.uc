...
function InitControls()
{
	...
	dialogAsset = Class'InterfaceClassic.UIControlDialogAssets'.static.InitScript(GetWindowHandle((dialogContainer.m_WindowNameWithFullPath $ ".UIControlDialogAsset")));
...
function UpdateUISlotControls()
{
	...
	while((i < _virtualSlots.Length))
	{
		...
		if(((tempSlotInfo.slotInfo.isBuffSlot == true) && (tempSlotInfo.slotInfo.ClassID > 0)))
		{
			...
			Class'InterfaceClassic.L2Util'.static.GetSkill2ItemInfo(SkillInfo, validItemInfo);
		}
	...
	while((i < Max(itemIndexList.Length, recordCnt)))
	{
		if((i < itemIndexList.Length))
		{
			...
			if(slotInfo.isBuffSlot)
			{
				...
				Class'InterfaceClassic.L2Util'.static.GetSkill2ItemInfo(SkillInfo, tmepItemInfo);
			}
			else
			{
...
function Rq_C_EX_VIRTUALITEM_SYSTEM(E_VI_UPDATE_TYPE UpdateType, int mainIndex, int SubIndex, int Slot, UIPacket._VirtualItemInfo vItemInfo)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_VIRTUALITEM_SYSTEM(stream, packet))
	{
		return;
	}
	...
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(875, stream);
	return;
}
...
function Nt_S_EX_VIRTUALITEM_SYSTEM_BASE_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_VIRTUALITEM_SYSTEM_BASE_INFO(packet))
	{
		return;
	}
...
function Rs_S_EX_VIRTUALITEM_SYSTEM()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_VIRTUALITEM_SYSTEM(packet))
	{
		return;
	}
...
function Nt_S_EX_VIRTUALITEM_SYSTEM_POINT_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_VIRTUALITEM_SYSTEM_POINT_INFO(packet))
	{
		return;
	}
	_uiInfo.GetPoint = packet.nTotalGetVISPoint;
...
event OnClickButton(string buttonStr)
{
	...
	{
		case "WindowHelp_BTN":
			Class'InterfaceClassic.HelpWnd'.static.ShowHelp(50000);
			break;
