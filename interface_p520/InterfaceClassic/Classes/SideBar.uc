...
event OnClickButton(string a_ButtonID)
{
	...
	{
		case "MinimizeButton":
			Class'InterfaceClassic.MinimizeManager'.static.Inst()._MinimizeWindow(m_hOwnerWnd.m_WindowNameWithFullPath);
			break;
		default:
			break;
	}
...
function DefaultShowHide()
{
	...
	if((getInstanceUIData().GetIsLiveServer() && Class'InterfaceClassic.WorldExchangeBuyWnd'.static.Inst().ChkUseableServerID()))
	{
...
function SortWindows()
{
	...
	if((isShowSideBar && !Class'InterfaceClassic.MinimizeManager'.static.Inst()._IsMin(m_hOwnerWnd.m_WindowNameWithFullPath)))
	{
		Me.ShowWindow();
	}
	else
...
function HandleDialogOK()
{
	...
	switch(Class'InterfaceClassic.UICommonAPI'.static.DialogGetID())
	{
		case 0:
...
function SetAlarmOnOff(int Index, bool On)
{
	...
	{
		Class'InterfaceClassic.MinimizeManager'.static.Inst()._ShowAlarm(m_hOwnerWnd.m_WindowNameWithFullPath);
	}
	else
	{
		Class'InterfaceClassic.MinimizeManager'.static.Inst()._HideAlarm(m_hOwnerWnd.m_WindowNameWithFullPath);
	}
	return;
}
...
function RT_S_EX_HOMUNCULUS_SIDEBAR()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_HOMUNCULUS_SIDEBAR(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_MABLE_GAME_UI_LAUNCHER()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_MABLE_GAME_UI_LAUNCHER(packet))
	{
		return;
	}
