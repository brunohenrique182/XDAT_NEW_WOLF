...
{
	abilitySwapBtnGroup = Class'InterfaceClassic.UIControlGroupButtonAssets'.static._InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".SubUIControlGroupButtonAsset")));
...
function ParsePacket_S_EX_ACQUIRE_AP_SKILL_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_ACQUIRE_AP_SKILL_LIST(packet))
	{
		return;
	}
	showDisable(false);
...
function API_C_EX_ACQUIRE_POTENTIAL_SKILL()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_ACQUIRE_POTENTIAL_SKILL(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(443, stream);
...
function API_C_EX_REQUEST_POTENTIAL_SKILL_LIST()
{
	...
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(444, stream);
...
function API_C_EX_RESET_POTENTIAL_SKILL()
{
	...
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(445, stream);
...
function API_C_EX_ABILITY_WND_OPEN()
{
	...
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(448, stream);
...
function API_C_EX_ABILITY_WND_CLOSE()
{
	...
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(449, stream);
...
function Rq_C_EX_CHANGE_ABILITY_PRESET(int Index)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_CHANGE_ABILITY_PRESET(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(849, stream);
	abilitySwapBtnGroup._setDelayTime(5500);
	abilitySwapBtnGroup._tryDelayClick();
