...
function OnbtnGiveUpCurrentQuestClick()
{
	...
	Class'InterfaceClassic.DialogBox'.static.Inst().AnchorToOwner(0);
	Class'InterfaceClassic.DialogBox'.static.Inst().DelegateOnOK = RQ_C_EX_QUEST_CANCEL;
	return;
}
...
function _OnbtnShowDialog()
{
	...
	if(isAcceptable)
	{
		Class'InterfaceClassic.QuestDialogWnd'.static.Inst()._ShowStartDialog(currentQuestID);
	}
	else
	{
		Class'InterfaceClassic.QuestDialogWnd'.static.Inst()._SohwDialogAgain(currentQuestID);
	}
	return;
}
...
{
	Class'InterfaceClassic.QuestDialogWnd'.static.Inst()._ShowCompleteDialog(currentQuestID);
	return;
}
...
function SetGoalText(string Goalname, int Cnt, int goalNum)
{
	...
	if(Class'InterfaceClassic.L2Util'.static.Inst().GetEllipsisString(goalnameEllipsed, (277 - numW)))
	{
...
function SetButtons()
{
	...
	if(isAcceptable)
	{
		...
		if(Class'InterfaceClassic.QuestDialogWnd'.static.Inst()._CheckDistanceToStart(currentQuestID))
		{
...
function RQ_C_EX_QUEST_CANCEL()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_QUEST_CANCEL(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(829, stream);
	return;
}
