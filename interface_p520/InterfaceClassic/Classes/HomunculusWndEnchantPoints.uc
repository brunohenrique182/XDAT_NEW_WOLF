...
function Handle_S_EX_HOMUNCULUS_GET_ENCHANT_POINT_RESULT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_HOMUNCULUS_GET_ENCHANT_POINT_RESULT(packet))
	{
		return;
	}
...
function Handle_S_EX_HOMUNCULUS_INIT_POINT_RESULT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_HOMUNCULUS_INIT_POINT_RESULT(packet))
	{
		return;
	}
	AddSystemMessage(packet.nID);
