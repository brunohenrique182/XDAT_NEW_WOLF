...
function HandleShowDialogAuto()
{
	...
	Class'InterfaceClassic.DialogBox'.static.Inst().AnchorToOwner(0, 170);
...
function API_C_EX_BLESS_OPTION_ENCHANT(int scrollClassID, int targetSID)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_BLESS_OPTION_ENCHANT(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(660, stream);
	return;
}
...
function API_C_EX_BLESS_OPTION_PROB_LIST(int nScrollClassID, int nItemClassID)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_BLESS_OPTION_PROB_LIST(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(925, stream);
	return;
}
...
function Handle_S_EX_BLESS_OPTION_PROB_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_BLESS_OPTION_PROB_LIST(packet))
	{
		return;
	}
	...
	if((packet.optionProbList.Length > 0))
	{
		...
		{
			GetMeRichListCtrl("Disable_Wnd.ItemBlessInfoWnd.Probability_RichListCtrl").InsertRecord(makeRecordProbability(getBlessEffectDesc(packet.nItemClassID, packet.optionProbList[i].nID), Class'InterfaceClassic.L2Util'.static.Inst().MakeDecimalPointString(string(packet.optionProbList[i].nProb), 5, true, true)));
			i++;
		}
	}
	if((packet.shapeProbList.Length > 0))
	{
		...
		{
			GetMeRichListCtrl("Disable_Wnd.ItemBlessInfoWnd.Probability_RichListCtrl").InsertRecord(makeRecordProbability(GetItemNameAllByClassID(packet.shapeProbList[i].nID), Class'InterfaceClassic.L2Util'.static.Inst().MakeDecimalPointString(string(packet.shapeProbList[i].nProb), 5, true, true)));
			i++;
		}
	}
	return;
...
function Handle_S_EX_OPEN_BLESS_OPTION_SCROLL()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_OPEN_BLESS_OPTION_SCROLL(packet))
	{
		return;
	}
	if(FindItem(packet.nScrollClassID, tItemInfo))
	{
		...
		PlaySound("ItemSound2.smelting.Smelting_dragin");
		GetProbability_Txt().SetText(((GetSystemString(13938) $ ":") @ Class'InterfaceClassic.L2Util'.static.Inst().MakeDecimalPointString(string(packet.nProb), 5, true, true)));
		GetMeTextBox("Disable_Wnd.ItemBlessInfoWnd.Probability_Txt").SetText(((GetSystemString(13938) $ ":") @ Class'InterfaceClassic.L2Util'.static.Inst().MakeDecimalPointString(string(packet.nProb), 5, true, true)));
...
function Handle_S_EX_BLESS_OPTION_ENCHANT()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_BLESS_OPTION_ENCHANT(packet))
	{
		return;
	}
