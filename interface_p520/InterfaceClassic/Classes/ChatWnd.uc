...
function HandleOnClickContextMenu(int channelID)
{
	...
	tabindex = Class'InterfaceClassic.UIControlContextMenu'.static.GetInstance()._GetReservedInt();
	i = 1;
	while((i < 4))
	{
		if((i == tabindex))
...
event OnTimer(int TimerID)
{
	switch(TimerID)
	{
		...
		case 2022:
			_Swap2AlphaNormal();
			Class'InterfaceClassic.SystemMsgWnd'.static.Inst()._Swap2Alpha();
			break;
		default:
			break;
	}
...
event OnMouseOver(WindowHandle W)
{
	...
	_Swap2FullAlphaNormal();
	Class'InterfaceClassic.SystemMsgWnd'.static.Inst()._Swap2FullAlpha();
	return;
}
...
function HandleOnClickContextMenuIcon(int Index)
{
	...
	UserName = Class'InterfaceClassic.UIControlContextMenu'.static.GetInstance()._GetReservedString();
	switch(Index)
	{
		case 0:
...
function ShowNewMessageBtn(int tabindex)
{
	if((tabindex == 5))
	{
		Class'InterfaceClassic.SystemMsgWnd'.static.Inst()._ShowNewMessageBtn();
		return;
	}
...
function API_C_EX_REQUEST_INVITE_PARTY(string param)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_REQUEST_INVITE_PARTY(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(752, stream);
	return;
}
