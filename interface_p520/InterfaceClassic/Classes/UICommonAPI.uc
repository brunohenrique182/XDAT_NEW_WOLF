...
{
	return new Class'InterfaceClassic.UICommonAPI';
}
...
{
	Class'InterfaceClassic.DialogBox'.static.Inst()._DialogShowHtmlWithTarget(modalType, dialogType, strMessage, strControlName);
	return;
}
...
function DialogShowHtml(UIScript.EDialogModalType modalType, UIScript.EDialogType dialogType, string strMessage, optional WindowHandle wnd)
{
	...
	Class'InterfaceClassic.DialogBox'.static.Inst()._DialogShowHtml(modalType, dialogType, strMessage, wnd);
	return;
}
...
{
	Class'InterfaceClassic.DialogBox'.static.Inst()._DialogShowWithTarget(modalType, dialogType, strMessage, strControlName);
	return;
}
...
function DialogShow(UIScript.EDialogModalType modalType, UIScript.EDialogType dialogType, string strMessage, optional WindowHandle targetWnd)
{
	...
	Class'InterfaceClassic.DialogBox'.static.Inst()._DialogShow(modalType, dialogType, strMessage, targetWnd);
	return;
}
...
{
	Class'InterfaceClassic.DialogBox'.static.Inst()._DialogMoveToCursor();
	return;
}
...
{
	return Class'InterfaceClassic.DialogBox'.static.Inst()._IsOwnerWindow(m_hOwnerWnd);
}
...
{
	Class'InterfaceClassic.DialogBox'.static.Inst()._inputLimit(inputLimit);
	return;
}
...
function toggleWindow(string a_ControlID, optional bool bFocus, optional bool bUseOpenCloseSound)
{
	...
	else
	{
		...
		if(Class'InterfaceClassic.MinimizeManager'.static.Inst()._IsMin(a_ControlID))
		{
			Class'InterfaceClassic.MinimizeManager'.static.Inst()._MaximizeWindow(a_ControlID);
		}
		else
		{
			Class'NWindow.UIAPI_WINDOW'.static.ShowWindow(a_ControlID);
...
function AddEllipsisString(out array<RichListCtrlDrawItem> drawitems, string Text, int MaxW, optional Color TextColor, optional bool bStrNewLine, optional bool useDot, optional int nPosX, optional int nPosY, optional string FontName)
{
	...
	Str = Text;
	bEllipsised = Class'InterfaceClassic.L2Util'.static.GetEllipsisString(Str, (MaxW - nPosX));
	if(useDot)
	{
...
function string GetNameHtml(ItemInfo item)
{
	...
	if(Class'InterfaceClassic.UIData'.static.Inst().GetIsLiveServer())
	{
		ItemName = Class'NWindow.UIDATA_ITEM'.static.GetRefineryItemName(item.Name, item.RefineryOp1, item.RefineryOp2);
	}
	else
...
{
	if(Class'InterfaceClassic.DialogBox'.static.Inst()._ChkFocus(m_hOwnerWnd, bFocused))
	{
		return;
	}
