...
function Initialize()
{
	...
	filterGroupButton = Class'InterfaceClassic.UIControlGroupButtonAssets'.static._InitScript(filterGroupButtonWindow);
...
function Rq_C_EX_PURCHASE_LIMIT_CRAFT_ITEM(int nSlotNum, int nItemAmount, optional int successionItemSId, optional int materialItemSId)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_PURCHASE_LIMIT_CRAFT_ITEM(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(930, stream);
	return;
}
...
{
	Class'InterfaceClassic.HelpWnd'.static.ShowHelp(67);
	return;
}
...
function HandleS_EX_PURCHASE_LIMIT_CRAFT_ITEM_LIST()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_PURCHASE_LIMIT_CRAFT_ITEM_LIST(packet))
	{
		return;
	}
	if(((int(_autoCraftInfo.uiState) == 1) || (int(_autoCraftInfo.uiState) == 2)))
...
function RichListCtrlRowData makeRecord(int Index, bool canBuy)
{
	...
	Class'InterfaceClassic.L2Util'.static.GetEllipsisString(productData.ProductName, 190);
...
function SetBuyItems()
{
	...
	while((i < productData.BuyItems.Length))
	{
		...
		if((i < 5))
		{
			GetCraftProbabilityTextBox(i).SetText(Class'InterfaceClassic.L2Util'.static.Inst().MakeDecimalPointString(string(shopItemData.probList[i]), 5, true, true));
		}
	...
	{
		Class'InterfaceClassic.ShopLcoinCraftMinimizeWnd'.static.Inst().SetItemSlots(productData);
	}
	return;
}
...
function SetFailWnd(int msgIndex, optional bool isSystemStr)
{
	...
	if((int(_autoCraftInfo.uiState) == 2))
	{
		Class'InterfaceClassic.ShopLcoinCraftMinimizeWnd'.static.Inst().SetFailMessage(msgIndex, isSystemStr);
	}
	return;
}
...
function SetCurrentCostItems(optional bool modifyList)
{
	...
	while((i < costItemInfos.Length))
	{
		...
		if(((costItemInfo.ItemClassID > 0) || (costItemInfo.ItemClassID == -800)))
		{
			...
			Class'InterfaceClassic.L2Util'.static.GetEllipsisStringWithAdd(Info.Name, Info.AdditionalName, 290);
...
function ItemInfo GetSkillItemInfo(int SkillID)
{
	...
	Class'InterfaceClassic.L2Util'.static.Inst().GetSkill2ItemInfo(sInfo, iInfo);
...
function OnReceivedCloseUI()
{
	if((int(_autoCraftInfo.uiState) == 1))
	{
		if(Class'InterfaceClassic.ShopLcoinCraftMinimizeWnd'.static.Inst().Me.IsShowWindow())
		{
			Class'InterfaceClassic.ShopLcoinCraftMinimizeWnd'.static.Inst()._isUserCancel = true;
		}
		ResultAutoCraft();
		return;
	}
...
function ResultAutoCraft()
{
	...
	Class'InterfaceClassic.ShopLcoinCraftMinimizeWnd'.static.Inst().UpdateAutoCraftInfo(_autoCraftInfo);
	return;
}
...
function StartAutoCraftTimer()
{
	...
	if(true)
	{
		Class'InterfaceClassic.ShopLcoinCraftMinimizeWnd'.static.Inst().StartCraftProgressBar();
	}
	KillAutoCraftTimer();
...
function CheckAndContinueAutoCraft(string param)
{
	...
	while((i < ItemCount))
	{
		...
		if((itemIndex < 5))
		{
			...
			Class'InterfaceClassic.ShopLcoinCraftMinimizeWnd'.static.Inst().UpdateBuySlot(itemIndex, tempCraftNum);
		}
		if((int(_autoCraftInfo.Type) == 1))
		{
			if((isLimitBuyType(productData) == true))
...
function SetAutoCraftMinimize(bool isMinimize)
{
	if(isMinimize)
	{
		...
		Class'InterfaceClassic.ShopLcoinCraftMinimizeWnd'.static.Inst().OpenWindow();
	}
	else
	{
		m_hOwnerWnd.SetVisibility(true);
...
function CloseAutoCraftMinimizeWnd()
{
	if(true)
	{
		Class'InterfaceClassic.ShopLcoinCraftMinimizeWnd'.static.Inst().CloseWindow();
	}
	return;
}
