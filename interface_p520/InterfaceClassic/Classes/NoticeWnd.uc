...
function clickNoticeButton(string logicIDStr, string param)
{
	...
	else if((logicID == 1))
	{
		...
		{
			Class'InterfaceClassic.QuestWnd'.static.Inst()._ToggleShowByNotice();
			return;
		}
...
{
	Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst()._CheckOnClickNoticeBtn();
	return;
}
...
function ParsePacket_S_EX_HOLY_FIRE_NOTIFY()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_HOLY_FIRE_NOTIFY(packet))
	{
		return;
	}
...
function RT_S_EX_WORLD_EXCHANGE_SELL_COMPLETE_ALARM()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_WORLD_EXCHANGE_SELL_COMPLETE_ALARM(packet))
	{
		return;
	}
	if(Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst().m_hOwnerWnd.IsShowWindow())
	{
		Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst().HandleCancelCancel();
		Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst().historyScr.HandleRefresh();
	}
	ParamAdd(param, "rewardCount", string(Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst().historyScr.receivedNum));
...
function RT_S_EX_WORLD_EXCHANGE_SETTLE_ALARM()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_WORLD_EXCHANGE_SETTLE_ALARM(packet))
	{
		return;
	}
	if(Class'InterfaceClassic.WorldExchangeRegiWnd'.static.Inst().m_hOwnerWnd.IsShowWindow())
...
function ParsePacket_S_EX_DETHRONE_DAILY_MISSION_COMPLETE()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_DETHRONE_DAILY_MISSION_COMPLETE(packet))
	{
		return;
	}
...
function ParsePacket_S_EX_PENALTY_ITEM_INFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_PENALTY_ITEM_INFO(packet))
	{
		return;
	}
...
function S_EX_PVPBOOK_SHARE_REVENGE_NEW_REVENGEINFO()
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Decode_S_EX_PVPBOOK_SHARE_REVENGE_NEW_REVENGEINFO(packet))
	{
		return;
	}
