...
function OnShow()
{
	if(true)
	{
		Class'InterfaceClassic.BottomBar'.static.Inst().SetPartyOnOffState(true, false);
	}
	else
	{
...
function OnMinimize()
{
	if(true)
	{
		Class'InterfaceClassic.BottomBar'.static.Inst().SetPartyOnOffState(false, true);
	}
	else
	{
...
function HandlePartyMatchRoomStart(string param)
{
	...
	if(Class'NWindow.UIAPI_WINDOW'.static.IsMinimizedWindow("PartyMatchRoomWnd"))
	{
		if(true)
		{
			Class'InterfaceClassic.BottomBar'.static.Inst().SetPartyAlarmOn(true);
		}
		else
		{
...
function OnHide()
{
	if(true)
	{
		Class'InterfaceClassic.BottomBar'.static.Inst().SetPartyOnOffState(false, false);
	}
	else
	{
...
function RequestPartyRoomAnnounce()
{
	if((getInstanceUIData().GetIsLiveServer() == false))
	{
		...
		Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(931, _emptyByteArray);
