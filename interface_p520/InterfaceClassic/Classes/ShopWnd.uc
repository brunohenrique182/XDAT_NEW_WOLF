...
event OnLoad()
{
	...
	uicontrolTextInputScr = Class'InterfaceClassic.UIControlTextInput'.static.InitScript(GetWindowHandle((m_Windowname $ ".FindText")));
		...
		if(((Id == 111) && (Num > INT64(0))))
		{
			...
			if((topIndex >= 0))
			{
				if((int(m_shopType) == 1))
				{
					...
					MAXITEMNUM = Class'InterfaceClassic.DialogBox'.static.Inst().inputLimit;
					if((Num > MAXITEMNUM))
					{
						Num = MAXITEMNUM;
					}
...
function RichListCtrlRowData makeRecord(ItemInfo iInfo)
{
	...
	Class'InterfaceClassic.L2Util'.static.GetEllipsisString(allName, wMax);
