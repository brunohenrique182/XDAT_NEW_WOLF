...
function requestCreateInfos()
{
	...
	{
		ReturnShowXMLDetailTooltip(true);
		Class'InterfaceClassic.MultiSellItemExchangeWnd'.static.Inst().OpenWindow(Info.Id.ClassID, true);
		return;
	}
...
function API_C_EX_CREATE_ITEM_PROB_LIST(int nClassID)
{
	...
	if(!Class'InterfaceClassic.UIPacket'.static.Encode_C_EX_CREATE_ITEM_PROB_LIST(stream, packet))
	{
		return;
	}
	Class'InterfaceClassic.UIPacket'.static.RequestUIPacket(895, stream);
