...
function Initialize()
{
	scrollTester = Class'InterfaceClassic.UIControlTilelist'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".ScrollAreaWnd")), 3, 2);
	...
	scrollTesterV = Class'InterfaceClassic.UIControlTilelist'.static.InitScript(GetWindowHandle((m_hOwnerWnd.m_WindowNameWithFullPath $ ".ScrollAreaWndV")), 2, 3, true);
...
function PlayAnimTextureHandle(string BTNID, int rendererIndex, int itemIndex)
{
	...
	rotateTObject = Class'InterfaceClassic.L2UITween'.static.Inst()._AddTweenRotate(GetAnimTextureHandle((scrollTester._GetRendererPath(rendererIndex) $ ".circleEffect")), 9, 370.0000000, 2000.0000000);
	rotateTObject._Play();
	return;
}
...
function PlayAnimTextureHandleV(string BTNID, int rendererIndex, int itemIndex)
{
	...
	l2UITweenScript = Class'InterfaceClassic.L2UITween'.static.Inst();
	rotateTObject = Class'InterfaceClassic.L2UITween'.static.Inst()._AddTweenRotate(GetAnimTextureHandle((scrollTesterV._GetRendererPath(rendererIndex) $ ".circleEffect")), 2, -490.0000000, 4000.0000000, 0.0000000, true);
	...
	Class'InterfaceClassic.L2UITween'.static.Inst().AddTweenObject(tweenObj);
	rotateTObject._Play();
	return;
}
