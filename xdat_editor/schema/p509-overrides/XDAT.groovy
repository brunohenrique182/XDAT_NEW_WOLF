...
class XDAT implements IOEntity {
    ...
    byte[] modernTrailingData = new byte[0]
    ...
    XDAT read(InputStream input) {
        ...
        modernTrailingData = input.readAllBytes()
    ...
    XDAT write(OutputStream output) {
        ...
        if (modernTrailingData != null && modernTrailingData.length > 0)
            output.write(modernTrailingData)
